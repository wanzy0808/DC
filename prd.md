...
### 7.2.3d — Brief visual Zen Atelier (24 September 2026)

Owner menetapkan `template.md` sebagai spesifikasi terperinci **khusus Zen Atelier** untuk pencocokan langsung dengan `Zen Atelier Wedding Moodboard UI.png`: 15 kontrol (`envelope`, 13 section semantik, `music` global), desain kertas krem editorial, animasi teks dan galeri masonry/carousel/parallax yang terkurasi, tanpa label `Pratinjau` di dalam undangan, tanpa CTA `Lihat Undangan` setelah amplop dibuka, serta tombol masuk berlabel **Buka Undangan**. Hapus teks filler teknis seperti `Preview mengikuti renderer undangan publik` dari antarmuka katalog/Studio sambil mempertahankan pemberitahuan penting tentang error, data, dan akses. `template.md` menjadi brief implementasi desain, bukan bukti fitur telah dikerjakan atau persetujuan mengubah landing/Pintu/template lain. Renderer/data/RSVP/media/musik tetap mengikuti kontrak bersama pada §7.2.3a–c dan aturan AGENTS.
