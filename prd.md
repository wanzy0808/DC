...
### 25 September 2026 — Menu Studio dipindah ke pinggir kanan frame

**Permintaan owner:** seluruh menu yang sebelumnya berada di kiri harus dipindahkan ke **ujung kanan frame Studio**, bukan sekadar tampil di kanan tengah canvas. **Implementasi:** urutan layout `InvitationDesigner.tsx` menjadi **canvas → inspector properti → rail menu paling kanan**, dengan seluruh menu Template/Bagian/Warna/Font/Isi/Foto/Aset/Teks/Musik tetap tersedia. Kontrol kontekstual layer dipindah dari posisi absolut yang menempel samping preview ke bagian atas inspector paling kanan. `studio.css` mengubah grid desktop pada lebar biasa dan ≥1280px serta kondisi inspector tertutup, memindahkan border/hover/active ke sisi kanan, dan menjaga rail horizontal di atas panel pada mobile. Toggle inspector berada di ujung kanan toolbar canvas dan menggunakan ikon arah kanan. Uji kontrak urutan DOM/layout ditambahkan di `tests/invitation-asset-layers.test.mjs`; `studio.md` diperbarui. **Batas:** tidak mengubah renderer undangan, isi/form, data event, desain template, atau perilaku drag layer. **Validasi:** commit source sudah disimpan; build Next.js dan verifikasi desktop/mobile nyata belum dinyatakan PASS.
