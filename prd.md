...
Masuk (`/login`) dan Daftar (dialog yang dibuka dari menu navbar maupun tombol Daftar di login) memakai identitas UI publik DC Organizer yang sama: tipografi Cinzel untuk heading, Fauna One untuk label/form, warna Rose sebagai aksen, permukaan popup putih dengan glow pink tipis dalam kedua mode (bukan card gelap saat Dark Mode), outline Rose, CTA Rose, dan kolom input serta tombol berbentuk persegi panjang bersudut bulat 16px sesuai satu token global `--dc-control-radius`. Kartu login lebar baca sekitar 480px di tengah halaman yang responsif; dialog pendaftaran lebar sekitar 490px dengan scroll internal ketika tinggi layar terbatas agar kolom, checkbox dan tombol dapat dijangkau di mobile. Navbar, footer, latar/kelopak global dan brand `BrandWordmark` tetap milik layout bersama; jangan menduplikasi dekorasi/pemutar musik atau mengubah landing Pintu.
...
**Bentuk tombol terbaru:** Landing, Studio dan Dashboard memakai satu bentuk CTA: **persegi panjang bersudut bulat 16px**, tidak memakai pill/rounded-full. Semua komponen reusable mengambil `--dc-control-radius` dari `app/globals.css`; Studio tidak boleh membuat radius khusus untuk Amplop/Cover, filter foto, dropdown urutan atau tombol pencarian. Label di Light Mode putih untuk tombol Rose terisi, di Dark Mode hitam; tombol outlined tetap terbaca. Kecuali kontrol artistik/checkbox/radio/navbar yang memang memiliki geometri berbeda, tidak ada aturan bentuk tombol lain.
...
- Burger navigation mempertahankan surface netral dan label/ikon Rose di Light/Dark, bukan teks hitam/putih. Login/Daftar menggunakan popup putih, teks isi gelap, dan glow Rose tipis di kedua mode, dengan shared input persegi panjang bersudut bulat dan CTA Rose. Style burger dan kedua dialog saat ini dikunci sampai owner meminta perubahan spesifik.
...
- Rose petals di `components/Layout/RosePetalBackground.tsx` adalah protected visual element.

### 15.4.1 Pintu produksi; eksperimen V2 lama telah diarsipkan

**Baseline aktif adalah homepage `/` dengan empat pintu layanan dan komposisi yang sudah disetujui owner (§15.4.1b).** Clone `/jiplak` dan seluruh route `/pintu-lab`, preview serta engine khusus eksperimen telah dipensiunkan dan dihapus dari branch aktif atas instruksi owner tanggal 24 September 2026. Pintu marketing produksi, `LandingDoorScene.tsx` dan transisi tidak boleh diganti/dikurangi demi membangkitkan eksperimen. Catatan V1/V2 Three.js, target `pintu1.png`, screenshot dan persetujuan per tahap adalah riwayat penelitian desain, **bukan instruksi implementasi aktif**. Arsip `pintu3d.md` tersedia melalui Appendix A/Git. Eksperimen pintu baru hanya boleh dimulai dengan scope baru dan persetujuan owner, tanpa mengubah empat layanan produksi yang sudah berjalan.
...
### 15.4.2 Transisi navigasi Pintu — baseline landing yang berlaku

- Homepage `/` menggunakan empat tujuan resmi: Event Planner `/event-planner`, Undangan Digital `/d-invitation`, Guestbook `/guestbook`, dan Undangan Fisik `/undangan-fisik`. Hover/orbit hanya respons pemilihan dan **tidak** otomatis mengubah route; tombol `Masuk` menavigasi layanan yang benar setelah pintu terpilih terbuka.
- Saat masuk, kamera mendekati pintu asli; gambar layanan pada permukaan portal memudar menjadi cahaya Rose polos dan veil Rose tanpa gambar pintu mulai menutup scene **sebelum navigasi**. Jangan tampilkan pintu kedua, overlay daun/kusen berulang, gambar layanan kedua, atau pintu orbital lain saat kamera mendekat. Kamera tidak melewati backdrop datar hingga memperlihatkan objek belakangnya. Transisi visual harus menyatu dengan halaman tujuan tanpa kotak/flash yang tampak.
- Pertahankan efek suara ringan, navigasi URL/history yang nyata, kontrol keyboard/touch, fokus, scroll, cancel/error fallback, light/dark, ID/EN, performa mobile, dan `prefers-reduced-motion`. Halaman tujuan tetap dapat digunakan dan di-scroll seperti biasa.
- Source visual/interaksi yang berlaku adalah `app/page.tsx`, `components/Landing/Pintu/LandingDoorScene.tsx` dan `PortalTransition` yang dipakai aplikasi. Rencana lama berupa kamera terbang melewati ruang 3D laboratorium, integrasi tiga pintu, atau zoom-out seluruh halaman **tidak otomatis menjadi requirement untuk merombak implementasi final**. Ubah landing hanya atas instruksi owner yang spesifik; catatan eksperimen lama tersimpan di Git history.
...
**Catatan historis (22 September 2026, bentuk pill sudah diganti oleh kontrak rounded-rectangle 24 September):** Fokus saat itu pada `/template-design?template=romantic-rose`: tombol Lihat Undangan/kolom cari ber-outline Rose dan dropdown Urutkan A–Z/Z–A.
...
**Feedback owner saat itu (bentuk pill telah digantikan aturan 24 September 2026):** Jangan membuat file style hanya untuk `/template-design`; satu bentuk sudut yang dibulatkan dan outline Rose perlu konsisten melalui shared controls.
...
### 24 September 2026 — Satu bentuk tombol rounded rectangle mengikuti landing

**Koreksi owner:** Tombol aplikasi DC Organizer bukan kapsul/pill. Bentuk yang diminta adalah **kotak/persegi panjang dengan keempat sudut membulat** seperti gaya tombol landing. Owner meminta agar aturan sebelumnya yang bertentangan tidak lagi membingungkan dan Studio mengikuti bentuk yang sama.

**Perubahan source of truth:** `app/globals.css` menetapkan **`--dc-control-radius: 16px`** untuk tombol/input/filter/select dan **`--dc-control-menu-radius: 18px`** untuk panel dropdown; `components/ui/button-variants.ts`, `components/ui/input.tsx`, `components/ui/control-styles.ts` sudah mengambil token global, sehingga tombol landing (komponen `Button`) dan tombol aplikasi mengikuti satu bentuk tanpa stylesheet warna/radius kedua. `components/InvitationStudio/InvitationDesigner.tsx` mengganti radius tombol Amplop/Cover dengan token global, `components/InvitationStudio/DesignerPanels.tsx` menyelaraskan search action/penanda selected, `components/InvitationStudio/studio.css` menyelaraskan icon action, dan tombol tutup dialog `app/template-design/page.tsx` disesuaikan. Warna teks Light/Dark dan jarak panah dropdown yang telah disetujui tidak berubah. `AGENTS.md` menghapus aturan preskriptif lama yang menyebut pill dan menetapkan satu rounded rectangle sebagai kontrak aktif; teks pembahasan bentuk pill yang tersisa dalam riwayat perubahan ini **historis, bukan aturan aktif**. Tidak menyentuh landing Pintu, artwork undangan, data, publikasi atau desain template.

**Validasi:** regresi bentuk radius reusable/control Studio perlu diverifikasi melalui tes dan GitHub Actions pada commit HEAD. Pengecekan screenshot landing dan Studio pada viewport desktop/mobile tetap penting untuk final visual.


### 24 September 2026 — Menu Isi Studio menjadi editor narasi template yang benar-benar tersedia

**Permintaan owner:** Ikon **Isi** pada rail Studio harus mengganti tulisan doa/permohonan/penjelasan yang muncul di undangan, bukan menjadikan setiap node teks editable atau menduplikasi data acara yang telah terikat pada database/komponen. Input nama orang tua, tanggal, venue, judul, label tombol, status dan informasi RSVP tetap pada sumber datanya masing-masing.

**Implementasi:** `lib/templates/editable-copy.ts` menambahkan registry whitelist `greeting`, `closing`, `zenQuote`, batas karakter, default copy per tema/event, serializer/parser `::copy=` yang terikat design key dan kompatibel dengan undangan lama. `components/InvitationStudio/designer-types.ts` dan `designer-state.ts` mengikutkan `copy` dalam key sehingga Undo/Redo, dirty state dan Save Design bekerja tanpa mengubah kolom description/identitas. `components/InvitationStudio/DesignerPanels.tsx` mengganti ContentPanel dari editor hashtag/dress code + tampilan ulang event menjadi input narasi yang dipakai renderer sungguhan; ZenQuote hanya muncul untuk Zen Atelier dengan kategori acara berformat pasangan. `components/InvitationStudio/InvitationDesigner.tsx` menerapkan perubahan teks langsung pada state/canvas, meneruskan `copy` dalam designKey, mengosongkan teks khusus tema saat memilih tema berbeda, dan tidak mengirim field DB event dari menu Isi. `components/InvitationStudio/InvitationPreview.tsx`, `UniversalInvitationTemplate.tsx` dan `RomanticRoseTemplate.tsx` menerima override belum disimpan saat preview serta membaca override tersimpan lewat `invitation.templateKey` pada URL tamu. Kedua renderer tetap fallback ke data `Invitation.description`/copy asli ketika belum ada override. Quote khusus Zen tetap berada pada bagian yang memang memiliki kutipan; tidak menambah paragraf doa fiktif ke tema lain. `tests/invitation-editable-copy.test.mjs` memeriksa batas, whitelist, roundtrip, integrasi Studio, serta renderer; `tests/studio-ui-cleanup.test.mjs` diperbarui agar tidak mengharuskan data acara di panel Isi. `template.md` dan `AGENTS.md` selaras dengan kontrak kanonik §7.2.1e.

**Batas:** Belum menambah field naratif baru ke tema yang tidak memilikinya, tidak membuat UI edit di dalam canvas per-teks, dan tidak memindahkan form hashtag/dress code ke Dashboard; nilai yang sudah tersimpan tetap dirender. Tidak mengubah pembayaran, status Publish, DB schema, asset, RSVP atau isi event yang sudah ada. CI HEAD dan inspeksi visual perlu diverifikasi terpisah.


### 24 September 2026 — Wajib komponen narasi editable pada setiap template baru

**Klarifikasi owner:** Aturan menu Isi harus menjadi **rule produksi semua template** sehingga tidak ada tema baru yang lupa menyiapkan komponen narasi untuk diganti pemilik undangan. Pembuka/salam, permohonan kehadiran, doa/harapan, dan penutup wajib diwujudkan sebagai slot teks nyata **di dalam** section yang sudah ada, bukan tambahan section baru atau textarea Studio yang tidak tersambung. Kata penjelasan dan kutipan yang benar-benar ada pada desain juga dapat menjadi slot tambahan; nama/jadwal/tempat/identitas tamu serta label komponen tetap dari sistem. `template.md` mencatat tabel slot, batas izin edit dan checklist READY, sedangkan §7.2.1e menetapkan kontrak kanonik. **Status:** dokumentasi aturan template baru selesai; renderer tema lama saat ini belum seluruhnya mempunyai `attendanceRequest` dan `prayerWish` terpisah. Jangan menyatakan fitur tersebut sudah tersedia pada semua template lama sebelum implementasi dan pengujian.


### 24 September 2026 — Our Story / Tentang Kami dapat diisi lewat menu Isi Studio

**Permintaan owner:** Tambahkan segmen khusus kisah perjalanan pasangan pengantin, berjudul **Our Story / Tentang Kami**, yang narasinya diganti pada ikon **Isi** di Studio. Ini bukan penggandaan nama, tanggal atau data pasangan.

**Implementasi:** `lib/templates/editable-copy.ts` menambah slot opsional `ourStory` khusus kategori pasangan pada `availableEditableCopyFields`, batas 1.600 karakter, tanpa cerita default fiktif. `components/InvitationStudio/DesignerPanels.tsx` menampilkan textarea `Our Story / Tentang Kami` hanya untuk acara pasangan, mendukung teks panjang/baris baru. `components/PublicInvitation/OurStorySection.tsx` adalah subsegmen presentasi bersama yang mempertahankan warna/font/komposisi tema (khusus Romantic Rose serta versi universal termasuk Zen Atelier), judul tetap komponen dan konten teks user tanpa HTML. `components/PublicInvitation/UniversalInvitationTemplate.tsx` dan `RomanticRoseTemplate.tsx` menampilkannya **setelah bagian Identity dan sebelum Event**, hanya jika Identitas ON dan cerita terisi; mode non-pasangan maupun teks kosong tidak memunculkan segmen. Penyimpanan memanfaatkan `::copy=` di design key yang sama, sehingga tidak membutuhkan tabel/database baru dan tetap konsisten antara canvas, undangan tamu serta Undo/Redo. Tidak mengubah lima belas toggle, Cover, Amplop, data acara, RSVP atau pembayaran. `tests/invitation-editable-copy.test.mjs` memeriksa whitespace/limit/roundtrip serta integrasi renderer. `template.md` mewajibkan subsegmen ini pada tema pasangan berikutnya tanpa menjadikannya kewajiban pengguna untuk menulis cerita.

**Validasi:** kode dan tes otomatis pada head diperiksa; perbandingan visual desktop/mobile dengan berbagai palet dan cerita panjang masih memerlukan browser. Tidak mengklaim semua template sekarang mempunyai cerita default; segmen memang tersembunyi sampai pemilik mengisi.


### 24 September 2026 — Thumbnail tema Studio ditampilkan utuh dalam format portrait

**Masalah/permintaan owner:** Thumbnail dua kolom di kiri Studio terlalu kecil dan hanya memperlihatkan potongan Cover; pemilik perlu melihat komposisi gambar undangan secara utuh sebelum memilih. **Perubahan:** `components/InvitationStudio/DesignerPanels.tsx` menggunakan satu kolom kartu portrait dengan nama dan label Dengan foto/Tanpa foto di bawah gambar, bukan badge yang menutupi desain. `components/Templates/TemplateGalleryCanvas.tsx` menambahkan mode `studio` yang mengukur lebar kartu melalui ResizeObserver dan mengecilkan renderer Cover asli secara proporsional ke format 9:19.5, tanpa mengganti renderer dengan foto statis. Mode galeri publik dan featured tidak diubah. Tidak ada perubahan database, template key, renderer undangan publik, pemilihan tema, atau penyimpanan desain. **Commit implementasi:** `5d1b8acc`, `f5fde0d3`. **Validasi:** source perubahan diperiksa; build/CI dan pemeriksaan browser desktop/mobile belum diverifikasi. Pastikan tema dengan Cover panjang dan pendek tetap tidak memotong konten penting di layar Studio.


### 24 September 2026 — Perbaikan label Ucapan Tamu sekaligus aktivasi pengiriman ucapan

**Permintaan owner:** Hilangkan gabungan label/keterangan `Ucapan TamuPengiriman ucapan belum tersedia.` pada Studio dan benahi fitur, bukan hanya menyembunyikan teks sementara fungsi tetap palsu. `DesignerPanels.tsx` menghapus kalimat unavailable. `GuestWishes.tsx` kini menampilkan form nama/ucapan dan daftar terbaru dari API pada undangan tamu, serta formulir nonaktif tanpa API call untuk canvas Studio/katalog. `UniversalInvitationTemplate.tsx` dan `RomanticRoseTemplate.tsx` sama-sama memakai komponen tersebut, tetap tunduk pada toggle `wishes` existing. `prisma/schema.prisma` menambah `GuestWish` terkait Invitation, SQL migrasi `20260924183000_guest_wishes`, dan `app/api/invite/[slug]/wishes/route.ts` menambah GET/POST dengan validasi event-scoped, publish, entitlement, password, panjang dan rate limit; **tidak menambah record Guest atau memodifikasi RSVP**. `tests/guest-wishes.test.mjs` menjaga sambungan renderer/preview dan otorisasi secara source-level; `template.md` dan `AGENTS.md` disesuaikan agar tidak ada aturan aktif yang masih menyebut Wishes placeholder. Sebelum operasi database nyata, jalankan migrasi target; tidak ada bukti migrasi live atau uji kirim end-to-end database dari workflow build saja.


### 24 September 2026 — Penyesuaian ukuran kartu preview tema di Studio

**Permintaan owner:** Preview tema sebelumnya sudah utuh namun terlalu besar. Kartu portrait panel kiri kini dibatasi lebar maksimum 190px (sekitar 5 cm CSS), tinggi proporsional sekitar 412px, satu kolom dan terpusat agar tetap bisa dilihat lengkap di panel sempit. `components/InvitationStudio/DesignerPanels.tsx` saja yang disesuaikan; scaling asli, lazy renderer dan canvas utama tidak diubah. **Commit final:** `607a328e` (menggantikan percobaan dua kolom `995ed876`). **Validasi:** perubahan kode GitHub tersimpan; QA browser dan CI belum diperiksa.


### 24 September 2026 — Dua thumbnail portrait per baris di Studio

**Permintaan owner:** Setelah versi satu kolom 190px, panel kiri masih cukup untuk dua pilihan tema yang berjajar; tiap thumbnail diharapkan sekitar 4 × 10 cm atau lebih kecil menurut ruang. **Implementasi:** `components/InvitationStudio/DesignerPanels.tsx` menjadi grid dua kolom, gap 8px, kartu maksimal 152px dan `min-w-0`, teks nama wrap, serta renderer `studio` tetap mengisi lebar kartu menggunakan aspek 9:19.5. Kartu menyusut ketika panel sempit; ukuran riil bergantung viewport/browser dan tidak memaksakan lebar 4 cm yang dapat menyebabkan overflow. Perubahan tidak menyentuh canvas utama, publikasi, renderer, data, filter atau pemilihan. **Commit:** `8c498a2f`. **Validasi:** source GitHub diperbarui; hasil browser dan build/CI belum diverifikasi.


### 24 September 2026 — Aset ilustrasi dan layer khusus Cover pada Studio

**Permintaan owner:** Menambah ikon **Aset** pada rail Studio, menampilkan file gambar yang sudah berada di `public/template`/`public/templates`, lalu menempelkannya pada canvas undangan dengan layer dan transparansi seperti affordance editor rujukan, tetapi menggunakan identitas desain DC Organizer sendiri. **Implementasi:** `app/api/templates/assets/route.ts` memindai gambar publik untuk sesi aktif; `components/InvitationStudio/AssetPanel.tsx` menampilkan galeri pencarian/batch, daftar lapisan dan slider; `components/PublicInvitation/InvitationAssetLayers.tsx` menampilkan/drag elemen dekorasi di Cover untuk tema universal/Romantic Rose. `lib/templates/asset-layers.ts` menormalisasi hingga 12 layer dan menyimpan nilainya di `::layers=` design key yang sudah event-scoped. `designer-state.ts`, `designer-types.ts`, `InvitationDesigner.tsx`, `InvitationPreview.tsx` menghubungkan preview nyata, Undo/Redo, toolbar opacity/urutan/hapus, dan Save yang sudah ada. `studio.css` menggunakan Rose token yang sama; aturan branding Pintu/dashboard, foto asli tamu, 15 visibilitas, RSVP, entitlements dan data event tidak dimodifikasi. **Batasan yang memang disengaja:** baru Cover, bukan manipulasi bebas seluruh section/teks/animasi. Tidak menduplikasi layanan template eksternal maupun menambah database. **Commit implementasi:** `6aaa2031` sampai `896ed7c8` (beberapa commit berurutan), dokumentasi final menyusul. **Validasi:** source telah diperiksa dan tes `tests/invitation-asset-layers.test.mjs` ditambahkan; build/CI, unggahan assets aktual, render hasil undangan berbayar, dan browser drag desktop/touch belum diverifikasi saat dokumentasi ditulis.


### 24 September 2026 — Stabilisasi implementasi Aset Studio

**Perbaikan setelah integrasi:** endpoint menggunakan `Dirent[]` eksplisit (`b5b53037`), parser URL ilustrasi menolak backslash/query/fragment/control dan encoding traversal ganda (`da237046`), pilihan tema yang masuk dari galeri juga memulai lapisan kosong seperti pergantian tema langsung (`50f5c774`), browser aset menampilkan tema aktif dahulu dalam batch kecil (`caf8ea31`, `32b6e136`), toolbar dan inspector memberi label **Opasitas** 0% transparan / 100% terlihat (`7d1696b1`, `5ab5e932`), dan tes kontrak mencakup symlink guard serta pergantian tema (`240d7fcd`). **Validasi:** inspeksi integrasi sumber via GitHub sudah dilakukan; status gabungan GitHub tidak memberikan laporan tes pada commit dokumentasi yang diperiksa. Build CI terbaru, browser desktop/touch, aset seluruh folder dan hasil undangan publik tetap **belum diverifikasi**; jangan menyatakan PASS. Tidak memerlukan migrasi Prisma.
...
### 24 September 2026 — Perbaiki referensi landing lama saat audit Markdown

Audit source menunjukkan homepage produksi sekarang dibangun langsung pada `app/page.tsx` dan memakai `SimpleDoorLab` dengan empat layanan. `app/pagecontoh/page.tsx` tidak ditemukan pada branch yang dibaca. Koreksi pasal aktif §15.4.1b, rujukan marketing/widget, dan keterangan V2 yang keliru menyebut tiga pintu produksi; rencana tiga pintu historis pada laboratorium tetap dapat ditelusuri tanpa menjadi izin mengubah desain aktif. Dokumen `README.md` dan `AGENTS.md` diselaraskan terpisah. Perubahan ini **dokumentasi saja**, belum memverifikasi QA browser atau CI.


### 24 September 2026 — Lengkapi kontrak pintu produksi pada PRD

Perbandingan §15.4.2 dengan renderer aktif `SimpleDoorLab.tsx` menegaskan empat tujuan, termasuk `/undangan-fisik`. Rumusan historis "tiga Pintu" dibatasi menjadi prototipe V2 laboratorium, tidak lagi instruksi mengurangi pintu pada landing produksi. Hanya dokumentasi diubah; tes browser/CI tidak dijalankan.


### 24 September 2026 — Selaraskan jumlah template dan status engine terhadap source

Pemeriksaan `lib/templates/catalog.ts` mengidentifikasi 12 tema bawaan: enam menggunakan foto dan enam tanpa foto, termasuk `pencil-reverie` yang belum tercantum pada daftar kanonik 11 tema sebelumnya. §7.2.3a–b, ringkasan font dan catatan Zen lama diselaraskan tanpa menghapus sejarah 10/11 tema pada Appendix A. §7.2.3c menegaskan upload audio baru harus berbasis binary sementara URL lama tetap kompatibel. `README.md` dan `AGENTS.md` perlu mengikuti angka dan sumber data yang sama. **Status:** audit source/dokumen saja; tidak ada perubahan runtime, tes CI/DB/browser tidak dijalankan.


### 24 September 2026 — Pemeriksaan akhir lintas 11 dokumen utama, tahap 1

**Dokumen diperiksa:** `prd.md`, `AGENTS.md`, `README.md`, `checklist.md`, `prd-tambahan.md`, `prd-landing.md`, `prdpaging.md`, `pintu3d.md`, `Dashboard-redesign.md`, `template.md`, `studio.md`; validasi data tertentu melawan `app/page.tsx`, `lib/templates/catalog.ts`, `lib/events/parents.ts`, `GuestWishes.tsx` dan Wishes API/migrasi. **Perubahan dokumen:** heading §7.2.0c ganda dipisahkan, §5.4a/§15.4.1b ditempatkan pada domainnya, kontrak empat pintu dan 12 tema disesuaikan source, README status Wishes diperbarui, dokumentasi legacy dibekukan, serta tautan perintah penulisan ke `prd1.md` diselaraskan. **Commit utama:** `ff7652c92bde2d960ef0016307d6490d13423d9a`, `e663066d274a60c6cb1c6caef28b6bb40810892f`, `fda3f25372d31afd6f4cb854091b7de59b7034d6`, `99881a320ec30c91f7097ab499e0d8597e6799ab`, `39f443650ac5c90dd0a659a9f20e086cccad6b70`, `44dc79bc5ae0d26c5babc5a1924f45b16742e2b5`, `c1d1661e35e4383419cee06b89b9c9fc083acc01`, `4c4554d9ee5c776299af49dd503b85e72ab3438f`, `7fb650554ec52faafa4ad18334e2a48efee2cc33`, `d682743ee7c43fe4ebdd039082518e1e9290fdcb`, `b2e29435a005468945522bb5f6bcb0e0a774fb60`, `b3bda907f008380fa8f399a553ff47263ab82ea5`, `cd621c123a1d4c7eb3afea3a0f22ec9fcd62e851`, `d3d3b0f70a16b8de1ba8376a25feba6a4cf2372e`, `ac243a267b2a979f8cae9c1c82161a07e5a920a5`, `4888cea706eada45b55bd2a6362e82e8283d278b`. **Pemeriksaan tekstual:** semua delapan assertion audit awal true (satu sumber aktif, nomor heading unik, pola lama terpilih tidak ada pada badan PRD, status arsip jelas, tidak ada instruksi ke `prd1.md` dalam tiga dokumen yang diperbaiki, README Wishes mutakhir, landing source selaras, serta katalog 12 = 6 foto + 6 tanpa foto). **Batas:** pemeriksaan source/Markdown saja; belum melakukan build/CI, browser, database produksi, audit semua Markdown nested, atau pembacaan menyeluruh semua klausul untuk menemukan seluruh kontradiksi. Tahap 2–4 tetap pending.

### 24 September 2026 — Konsolidasi empat dokumen lama ke PRD kanonik

**Keputusan owner:** karena landing telah jadi, empat sumber tambahan tidak lagi dipakai sebagai dokumen aturan aktif: `prd-tambahan.md`, `prd-landing.md`, `prdpaging.md`, `pintu3d.md`. Audit isi dilakukan terhadap §6, §7, §11, §15, §21, `AGENTS.md`, `README.md`, dan source landing terkini. Tiga dokumen pertama adalah delta historis dan draft yang belum seluruhnya disetujui; `pintu3d.md` adalah catatan rinci eksperimen V1/V2 serta persetujuan visual per tahap, bukan kontrak landing final. **Tidak menyalin rancangan tiga pintu, pagecontoh lama, instruksi lab, atau revisi sidebar yang sudah superseded sebagai persyaratan baru.** Aturan Dashboard, tipografi/copy, pilihan template, ilustrasi Pencil Reverie, dan integrasi tamu/RSVP sebelumnya telah tercatat di pasal PRD terkait atau panduan `template.md`. Satu detail yang ditambahkan ke badan PRD saat audit ini adalah §11.1a mengenai template pesan WA Blast per acara. §15.4.1 dan §15.4.2 dirapikan agar merujuk empat pintu dan transisi Rose landing yang sudah final, bukan target V2 lab yang belum mendapat persetujuan produksi.

**Arsip sumber penuh sebelum penghapusan (bisa dibaca melalui GitHub):**
- [PRD tambahan historis](https://github.com/wanzy0808/DC/blob/4c4554d9ee5c776299af49dd503b85e72ab3438f/prd-tambahan.md) — catatan Dashboard, percobaan landing, WA Blast dan Pencil Reverie;
- [Draft landing lama](https://github.com/wanzy0808/DC/blob/44dc79bc5ae0d26c5babc5a1924f45b16742e2b5/prd-landing.md) — opsi visual yang belum otomatis disetujui;
- [Draft halaman marketing lama](https://github.com/wanzy0808/DC/blob/c1d1661e35e4383419cee06b89b9c9fc083acc01/prdpaging.md) — arah komposisi, bukan instruksi mengubah halaman final;
...
### 24 September 2026 — Maintenance tahap lanjut: panel tema, orphan audit, dan aturan dokumen

**Permintaan owner:** periksa seluruh struktur repo, ikuti keputusan terbaru hanya jika aturan bertentangan, pertahankan aturan lama yang masih kompatibel, pisahkan komponen bila benar-benar meningkatkan maintenance, dan hapus file yang jelas tidak berguna tanpa merusak fitur. **Implementasi terlingkup:** `TemplatePanel` dipisah dari `DesignerPanels.tsx` ke `components/InvitationStudio/TemplatePanel.tsx`, namun tetap di-re-export dari jalur lama agar `InvitationDesigner` dan konsumen lain tidak berganti API; state cari/filter/sort/pagination, kelas visual, dan renderer kartu dipindahkan apa adanya. Tes source-level Studio dan kartu katalog membaca lokasi komponen yang baru. Workflow orphan audit sekarang dapat dijalankan manual dan pada perubahan source/aset di `main`/PR; hasil zero-inbound adalah **kandidat review**, bukan izin penghapusan otomatis, karena ada rute dan URL dinamis. `AGENTS.md`/`Dashboard-redesign.md` tidak lagi memerintahkan menulis file `prd-tambahan.md` yang sudah dihapus. Skrip `.github/scripts/modular-template-studio.py` yang berpotensi menulis ulang PRD lama dan beberapa berkas aplikasi telah dihapus pada commit `563abaf1da591c74037cad23dccbf99b873ccd6`, setelah diperiksa tidak dipanggil workflow/build atau skrip package. **Tidak ada perubahan logic invitation, database, landing atau renderer tamu. Validasi:** pemeriksaan source dan pengujian GitHub Actions untuk perubahan ini harus diperiksa pada commit terkait sebelum dinyatakan lulus. Penghapusan kandidat orphan lain dan pemecahan komponen besar ditunda sampai seluruh inbound import/URL, test dan kompatibilitas bisa dibuktikan.


### 24 September 2026 — Hapus 10 orphan source terverifikasi (tahap pembersihan 2)

Setelah workflow `Orphan Audit` run [36009224366](https://github.com/wanzy0808/DC/actions/runs/36009224366) mendeteksi file tanpa static inbound import, sepuluh kandidat berikut dibandingkan dengan daftar tree dan 16 berkas tes saat ini; semuanya bukan file route atau public asset dan tidak dibaca langsung oleh tes: `components/Landing/Pintu/AssetDoorPreview.tsx`, `Door40k.tsx`, `DoorFlorals.tsx`, `DoorRelief.tsx`, `PintuSecBaru.tsx`, `PintuSection.tsx`, `components/Layout/DreamWorldBackground.tsx`, `JiplakSweetBackground.tsx`, `components/InvitationStudio/GuestManagement.tsx`, `data/templates/showcase.ts`. Semuanya dihapus dari branch aktif dalam satu commit; sumber historis tetap berada dalam riwayat Git. Komponen Guest Management aktif tetap melalui modul Dashboard saat ini; tidak ada skema/record tamu dihapus. **Sengaja ditahan:** `.d.ts` pasangan JS, primitives UI umum, route eksperimen yang masih merupakan route publik, public media yang dapat dicari via Studio atau URL event lama, dan source lain yang belum cukup diverifikasi. **Validasi:** audit referensi dan tes source-level menunjukkan kandidat, sedangkan hasil build/CI setelah commit pembersihan harus diperiksa tersendiri. Tidak ada perubahan fungsi di komponen produksi yang tetap dipakai.


### 24 September 2026 — Lanjutan pembersihan dependensi yatim

Audit otomatis ulang pada [run 36010088126](https://github.com/wanzy0808/DC/actions/runs/36010088126) menemukan enam berkas pendukung yang tidak memiliki inbound static import setelah komponen lama dihapus: `components/InvitationStudio/GuestManagementPanels.tsx`, `components/InvitationStudio/guest-management-client.ts`, `components/Landing/Pintu/DreamPortalScene.tsx`, `components/Landing/Pintu/PintuCard.tsx`, `components/Landing/Pintu/asset-door-engine.js`, dan deklarasi pasangan engine tersebut `asset-door-engine.d.ts`. Enam belas berkas tes juga diperiksa: tidak ada pembacaan langsung berkas tersebut. Penghapusan ini khusus source yang tidak menjadi entry point App Router dan tidak mempunyai import masuk; fitur Dashboard tamu, tema undangan dan komponen Pintu produksi tetap utuh. `asset-door-segmentation.js` tidak otomatis dihapus bersama engine: referensi lintas source perlu diaudit ulang dahulu. **Validasi penghapusan:** build/CI pasca-commit harus diperiksa; jangan mengartikan orphan report sebagai bukti URL aset pelanggan tidak dipakai.
...
### 24 September 2026 — Audit silang lanjutan aturan UI dan catatan onboarding

Ditemukan sisa instruksi aktif di `AGENTS.md` §4 yang masih mewajibkan rail Rose solid, kanvas `#fff5f7` dan header selebar viewport; semuanya tidak sesuai koreksi owner terakhir dan `prd.md` §6. Kalimat-kalimat tersebut diselaraskan dengan sidebar putih/hitam dan mainframe bounded; navigasi lengkungan luar tetap dipertahankan. `README.md` dibersihkan dari instruksi interim kartu kecil/sidebar Rose dan status lama `Wishes` yang menyatakan pengiriman belum tersedia; status kini adalah API `GuestWishes` ada, tetapi migrasi target/QA nyata tetap wajib. Rujukan proteksi rose petals yang salah ketik diperbaiki ke source asli `RosePetalBackground.tsx`. Nama preview Denny & Christine dicatat sebagai aturan terbaru menggantikan referensi historis Zen. Tidak ada perubahan aplikasi, migrasi, route, atau file aset pada commit dokumentasi ini; validasi sebatas pembacaan source/dokumen dan koreksi string, bukan uji browser.

### 24 September 2026 — Modularisasi panel layer Studio dan countdown undangan

**Studio:** commit [`3eea60bc`](https://github.com/wanzy0808/DC/commit/3eea60bc54b17e6005af31b12f08d09f4102718f) memindahkan markup panel kontrol layer ilustrasi Cover dari `InvitationDesigner.tsx` ke `AssetLayerInspector.tsx` sebagai komponen UI presentasional. Pemilik state, pemilihan layer, koordinat drop, histori Undo/Redo, otorisasi dan penyimpanan tetap di `InvitationDesigner`; props hanya meneruskan kontrol UI yang sudah ada. Tes source-level yang membaca panel dipindah ke path baru. [Build Validation](https://github.com/wanzy0808/DC/actions/runs/36016774489) dan [Orphan Audit](https://github.com/wanzy0808/DC/actions/runs/36016774492) berhasil untuk commit ini, bukan bukti drag-drop/mobile/E2E browser.

**Renderer:** commit [`e89d8300`](https://github.com/wanzy0808/DC/commit/e89d83004bcc7bc5e180f34c85f1fbd5f27d3080) memusatkan aritmetika countdown ke `lib/invitations/countdown.ts` untuk Universal dan Romantic Rose; wrapper UI, label dan struktur section kedua renderer tetap terpisah. Tes baru memeriksa nilai countdown, tanggal tidak sah dan referensi kedua renderer. Build pertama [run 36017055819](https://github.com/wanzy0808/DC/actions/runs/36017055819) gagal karena assertion source-test memeriksa pola JSX `{countdown &&` yang tidak pernah dipakai Romantic Rose (kode aktual memakai `now !== null && countdown ?`), bukan karena kalkulasi countdown. Koreksi tes ada di commit [`32c455cd`](https://github.com/wanzy0808/DC/commit/32c455cdebb71a57842c2a80e38416883d49d25a); Build Validation commit koreksi [run 36017096159](https://github.com/wanzy0808/DC/actions/runs/36017096159) **success**, mencakup tes otomatis dan build setelah perbaikan assertion; Orphan Audit commit source pertama [run 36017055673](https://github.com/wanzy0808/DC/actions/runs/36017055673) juga success. Perubahan ini tidak mengubah model/data acara, guest, musik, fitur Studio atau desain tema; browser, tanggal beragam timezone dan database produksi belum diuji.

**Audit penghapusan:** tidak ada source/aset tambahan dihapus di tahap ini. Dua MP3 Zen dengan blob sama tetap dipertahankan karena default tema dan `musicUrl` historis dapat merujuk path berbeda; `/jiplak`, `/pintu-lab`, berkas `.d.ts` engine terkait dan `components/ui/sheet.tsx` juga tetap ada hingga dependensi runtime/URL terbukti aman. `checklist.md` §22 mencatat status dan risiko lanjutan.

### 24 September 2026 — README kembali menjadi panduan onboarding, bukan changelog kedua

Audit struktur repo terkini menunjukkan bagian `Repository Structure` di README masih menunjuk `components/InvitationStudio/GuestManagement.tsx`, `GuestManagementPanels.tsx`, `guest-management-*.ts` dan `data/templates/showcase.ts` yang telah dihapus pada batch orphan sebelumnya. Struktur diperbaiki menurut tree `main`: Guest Management aktif melalui `components/Dashboard/`, katalog tunggal di `lib/templates/catalog.ts`, dan Studio memuat `TemplatePanel.tsx` serta `AssetLayerInspector.tsx`. Dated milestones setelah `Documentation Governance` dipangkas menjadi peta fitur aktif yang merujuk pasal kanonik PRD dan file source, bukan menyalin ulang aturan desain yang sudah ada di §6/§7/§15. Penjelasan OAuth Google beserta konfigurasi redirect URI, migrasi database, runtime onboarding, status QA terpisah dan batas kompatibilitas media tetap dipertahankan. Riwayat utuh README lama tetap tersedia pada commit induk Git. Hanya dokumentasi; tidak ada route/source/asset/database pengguna yang dihapus atau diubah. Validasi: pembandingan direktori dan keberadaan file di GitHub; build dan browser tetap pemeriksaan terpisah.

### 24 September 2026 — Owner menutup seluruh route eksperimen pintu

Owner menyatakan seluruh eksperimen pintu/landing selesai dan secara eksplisit meminta menghapusnya serta merapikan file terkait. Commit [`9d90ecf6`](https://github.com/wanzy0808/DC/commit/9d90ecf65e3e6c1149a2f5d3410c3bcfc06d35fd) menghapus 11 berkas di `app/jiplak/` dan `app/pintu-lab/`, lalu membersihkan kondisi khusus route di `PublicAtmosphere`, Navbar, mini navigator dan registri marketing. [Build Validation run 36019026551](https://github.com/wanzy0808/DC/actions/runs/36019026551) dan [Orphan Audit run 36019026605](https://github.com/wanzy0808/DC/actions/runs/36019026605) berhasil untuk commit pertama; Orphan Audit kemudian mengidentifikasi root orphan preview CSS, procedural, orbital dan backdrop jiplak.

Commit [`f0368103`](https://github.com/wanzy0808/DC/commit/f0368103e874425a248ec6f41e3cecebfa2b3bb3) menghapus 19 berkas pendukung yang hanya dipakai eksperimen tersebut, termasuk dua deklarasi `reference-door-*.d.ts`, rangkaian mesin `reference-door-*.js`, preview `Pintu3DPreview`, helper ornamen/lighting/foyer CSS dan AssetDreamBackdrop/AssetDreamPortalScene. Footer, floating marketing controls dan CSS dibersihkan dari conditional route lama. [Orphan Audit run 36019259332](https://github.com/wanzy0808/DC/actions/runs/36019259332) berhasil dan kini hanya menandai primitive reuse `components/ui/sheet.tsx` sebagai zero-inbound — dipertahankan dengan sengaja. [Build Validation source kedua run 36019259303](https://github.com/wanzy0808/DC/actions/runs/36019259303) **success**; seluruh tes otomatis dan build lulus untuk commit penghapusan source tersebut. Commit dokumentasi setelahnya [Build Validation run 36019550418](https://github.com/wanzy0808/DC/actions/runs/36019550418) juga **success**. **Tidak dihapus:** `app/page.tsx`, produksi `SimpleDoorLab.tsx`, `PortalTransition.tsx`, desain undangan, database/event, dan `public/` media (URL historis pelanggan masih mungkin merujuk aset). Berkas terhapus dapat dibaca kembali via Git history, tetapi aturan aktif tidak lagi memerintahkan mempertahankan/menyempurnakan route lab. `AGENTS.md`, README dan checklist diselaraskan dengan status source terbaru.

### 24 September 2026 — Audit semantik nama komponen dan jurnal
...
Audit penamaan menemukan `package.json` masih menggunakan nama bawaan `my-app` walau aplikasi produksi adalah DC Organizer. Field `name` package privat diperbarui menjadi `dc-organizer` dan tes nama repository menambahkan pemeriksaan metadata. `pnpm-lock.yaml` tidak berisi string `my-app`, sehingga lockfile/dependensi tetap identik; source Next.js, URL publik, schema/database dan konfigurasi pembayaran tidak diubah. `docker-compose.yml` masih memakai nama container `doc-postgres` dan volume `doc_postgres_data` yang historis: **jangan rename secara kosmetik** karena dapat mengganti identitas container atau mengaburkan akses ke volume PostgreSQL lokal yang sudah ada. Refactor database ini memerlukan keputusan/migrasi terpisah, bukan bagian dari perapian nama file.


### 24 September 2026 — Editor objek dekoratif lintas section, handle transformasi, dan teks desain

**Permintaan owner:** memperluas Studio yang sebelumnya hanya bisa menempel Aset di Cover dan mengubah ukuran/rotasi lewat slider: aset harus dapat diseret ke bagian lain, resize/rotasi langsung di canvas, serta ada **Teks sebagai objek desain**. **Implementasi:** `lib/templates/asset-layers.ts` memperluas codec lama secara backward-compatible dengan kind=text, section, rotation, fontSize, role font, dan warna tervalidasi; data default ilustrasi lama tetap dapat dibaca. `components/PublicInvitation/InvitationAssetLayers.tsx` menggunakan koordinat relatif per-section, gesture pointer capture + handle sudut untuk resize dan handle atas untuk putar; drop objek dari section lama dapat berpindah ke section tujuan. `components/InvitationStudio/TextObjectPanel.tsx` menambah menu Teks dekoratif dengan pilihan bagian aktif. `InvitationDesigner.tsx`, `InvitationPreview.tsx`, dan `AssetLayerInspector.tsx` menyambungkan pilih/drop/fokus section, properti objek, Undo/Redo dan Simpan Desain yang sudah ada. `UniversalInvitationTemplate.tsx` dan `RomanticRoseTemplate.tsx` sama-sama merender objek dekoratif pada seluruh section yang terlihat, termasuk Amplop dan Footer; bukan meniru struktur/master editor platform rujukan. `AssetPanel.tsx` menampilkan layer teks tanpa img rusak dan mendukung drop gambar ke section aktif; `designer-types.ts` menambah panel Teks; `tests/invitation-asset-layers.test.mjs` menambah uji kontrak/coding untuk round-trip layer lama/baru dan wiring renderer. `studio.md` membedakan pekerjaan event-scoped ini dari fitur master desainer yang belum jadi. **Kompatibilitas:** tidak mengubah event/foto/RSVP/gift, publikasi, schema Prisma atau master katalog, dan objek dekoratif teks tidak mengubah label sistem. **Validasi:** source GitHub diperbarui dan tes ditambahkan/diperbarui; build/CI, drag langsung di Chrome/mobile, overlay form, dan public preview reload **belum diverifikasi**. JANGAN klaim PASS sampai diuji nyata.


### 24 September 2026 — Penajaman implementasi layer multi-section dan preview Amplop

Perubahan lanjutan setelah fitur awal: renderer universal sekarang juga menempatkan objek dekoratif pada Amplop (`da268bcd`); drag objek mempertahankan titik genggam ketika berpindah di section asal, menerima drop section terdaftar, dan auto-scroll saat mendekati tepi viewport (`438ea727`). Drag gambar dari katalog juga dapat diarahkan ke section yang terlihat dengan auto-scroll dan tanpa membatasi awal gesture ke Cover (`11ef5b3d`). Font teks peran body mengikuti font tema sesungguhnya, tidak fallback font dashboard (`9e934980`). Switching ke Amplop melalui teks/layer memulai ulang preview interaktif agar state internal `opened` tidak menyembunyikan Amplop (`4837fbaf`, bug rekursi helper yang muncul saat refactor telah langsung dibenahi `37a32c03`). Pemilihan layer dari panel kiri menavigasi ke section asalnya (`7924dc5e`). Romantic Rose mempertahankan overflow asli komponennya; perubahan tambahan `overflow-hidden` yang tidak perlu dibatalkan pada section non-Cover (`7fdc2232`). Pengujian kontrak sudah diselaraskan dengan fungsi section baru (`ccfce74f`, `43059c15`, `c128fd8e`). **Status:** perubahan GitHub tersimpan dan source serta wiring telah diaudit; build Next.js/CI, gestur asli Chrome dan touch, serta pemeriksaan undangan publik belum diverifikasi. Jangan klaim fitur Studio Master Designer lengkap.

### 25 September 2026 — Lokasi komponen renderer artwork Zen Atelier

Audit nama/lokasi kode menemukan `assets/templates/zen-atelier/ZenArtwork.tsx` berisi komponen React yang dipakai `ZenAtelierScene.tsx` dan lazy import `UniversalInvitationTemplate.tsx`, bukan gambar mentah. Kode komponen dipindah tanpa mengubah markup, prop, media URL, atau urutan pemuatan ke `components/PublicInvitation/ZenAtelierArtwork.tsx`; kedua import, tes aset/konvensi, README aset dan dokumentasi aktif diperbarui. Nama berkas historis dalam catatan lama Appendix A dipertahankan sesuai commit aslinya. Tidak ada perubahan tema, ilustrasi, public asset, database, rute, atau hasil pengguna. [Build Validation untuk commit source 398e7e35](https://github.com/wanzy0808/DC/actions/runs/36070080135) dan [Orphan Audit](https://github.com/wanzy0808/DC/actions/runs/36070080107) sama-sama lulus; verifikasi visual browser/mobile dan media undangan pelanggan tidak dilakukan dalam audit penamaan ini.

### 25 September 2026 — Pemisahan lokasi entry Studio dari marketing

`StudioEntrySection.tsx` adalah panel memilih event yang dirender dari `app/studio/page.tsx`, bukan CTA marketing. File dipindah dari `components/DigitalInvitation/StudioEntrySection.tsx` ke `components/InvitationStudio/StudioEntrySection.tsx`; satu import route `/studio` dan dua referensi tes `template-card-cover.test.mjs` diselaraskan. `components/DigitalInvitation/StudioSection.tsx` tetap berada di folder marketing untuk CTA halaman undangan digital. UI, copy ID/EN, auth, template intent, URL, database dan perilaku pemilihan acara tidak diubah. Catatan histori PRD lama tetap menyebut lokasi sesuai commit pada masanya.


### 25 September 2026 — Perbaikan resize samping/atas-bawah Studio

**Masalah yang dilaporkan owner:** delapan handle visual sudah ada, tetapi titik tengah kiri, kanan, atas dan bawah sebelumnya tetap mengubah ukuran gambar secara proporsional seperti handle diagonal; pengguna tidak bisa mengubah lebar atau tinggi secara mandiri. **Perubahan:** `InvitationAssetLayers.tsx` sekarang memakai proyeksi pointer terhadap sumbu objek setelah rotasi dan menyimpan geometri `height` independen ketika resize dimulai: kiri/kanan hanya mengubah `width`, atas/bawah hanya `height`, sementara empat sudut mengubah kedua ukuran secara proporsional. Geometri layer lama tetap memakai tinggi intrinsik hingga pengguna memakai handle resize. `lib/templates/asset-layers.ts` menambahkan properti tinggi opsional yang tervalidasi untuk serializer design key; `AssetLayerInspector.tsx` menampilkan tinggi independen dan tombol pulihkan proporsi asli. `tests/invitation-asset-layers.test.mjs` ditambah uji regresi codec dan wiring handler, `studio.md` diperbarui agar tidak lagi menyatakan semua handle mengunci proporsi. Tidak ada perubahan data acara, publikasi atau master template. **Validasi:** kode telah di-push tetapi build Next.js dan QA interaksi browser/touch/rotasi belum diverifikasi; jangan klaim PASS.


### 25 September 2026 — Resizing bertumpu tepi berseberangan, bukan membesar dari tengah

**Koreksi pemilik:** saat titik kanan ditarik, hanya tepi kanan yang bergerak dan tepi kiri tetap; titik kiri menarik hanya tepi kiri; titik atas/bawah berlaku serupa. Sebelumnya gesture sudah mengubah lebar/tinggi secara independen, tetapi pusat objek tetap sehingga kedua sisi bergerak bersamaan. **Implementasi:** `lib/templates/object-resize.ts` berisi geometri resize terpusat: pointer diproyeksikan ke sumbu lokal sesuai rotasi, perubahan lebar/tinggi dihitung dari handle tertentu, posisi pusat X/Y digeser setengah perubahan ukuran ke arah sisi yang ditarik (dan ditransformasikan kembali ke koordinat section). Handle sudut tetap mengubah ukuran proporsional dengan menjaga sudut lawan selama tidak mentok batas; perbedaan skala section X/Y dan rotasi dihitung eksplisit. `InvitationAssetLayers.tsx` menggunakan helper yang sama untuk tampilan drag langsung dan saat pointer dilepas, menghasilkan satu mutasi Undo/Redo; tidak mengubah data undangan, file asli, desain publik, atau tipe layer. `tests/invitation-asset-layers.test.mjs` menguji nilai numerik kanan/kiri/atas/bawah, sudut, dan objek rotasi 90°. `studio.md` kini menyebut anchoring posisi tepi yang benar. **Status verifikasi:** perubahan disimpan ke GitHub; build Next.js serta percobaan mouse/touch browser nyata belum dikonfirmasi PASS.


### 25 September 2026 — Klik area kosong canvas untuk melepas seleksi objek

**Permintaan owner:** setelah selesai mengedit gambar/teks, klik bidang canvas kosong harus langsung menutup kotak seleksi dan handle (seperti pemilih objek pada editor desain), bukan harus memakai tombol Hapus atau menekan Escape. **Perbaikan:** `InvitationAssetLayers.tsx` memberi penanda khusus pada root objek dan semua handle agar event klik bisa mengenali objek. `InvitationDesigner.tsx` membatalkan `selectedLayerId` pada klik bidang kosong preview atau workspace canvas, tetapi mengabaikan klik pada objek, panel inspector, tombol, tautan, input, select, textarea, area contenteditable, dan kontrol interaktif undangan. Ini hanya perubahan UI lokal; tidak menghapus aset/teks, tidak mengubah posisi, tidak membuat riwayat Undo, dan tidak mengubah data tersimpan maupun undangan publik. Pengujian kontrak click-away ditambahkan pada `tests/invitation-asset-layers.test.mjs`, serta aturan interaksi dicatat di `studio.md`. **Validasi:** perubahan dipush ke main; build/tes runtime/browser masih perlu diverifikasi.
...
### 25 September 2026 — Judul undangan digabung ke header utama Studio

**Koreksi owner:** nama undangan yang sebelumnya sudah ditaruh rata tengah tetapi masih berada di baris toolbar kedua harus **masuk ke header utama** bersama tombol kembali, brand, dan toggle. **Implementasi:** `components/InvitationStudio/InvitationEditorPage.tsx` membaca judul dari response undangan yang sudah di-fetch untuk entitlement (tanpa request tambahan), menampilkan nama yang diformat sebagai `h1` di tengah header bersama navigasi kiri dan kontrol tema/bahasa kanan; fallback ketika undangan belum dimuat adalah `Studio`. `components/InvitationStudio/InvitationDesigner.tsx` menghapus judul ganda dari toolbar editor dan tetap mempertahankan tombol Default, Undo, Redo dan Simpan Desain di toolbar di bawah header. `components/InvitationStudio/studio.css` menggunakan grid tiga kolom untuk benar-benar memusatkan judul pada desktop dan satu baris judul tersendiri yang masih **di dalam header yang sama** pada viewport sempit agar tidak menabrak brand/toggle. `tests/invitation-asset-layers.test.mjs` memperbarui kontrak posisi judul. Tulisan status 'Desain tersimpan' tetap tidak ditampilkan di bawah judul; pemberitahuan simpan di area status tetap tidak berubah. **Validasi:** source GitHub disimpan; build dan QA tampilan desktop/mobile belum terverifikasi.


### 25 September 2026 — Hapus status idle “Siap diedit.” di kiri bawah Studio

**Permintaan owner:** pesan `Siap diedit.` di kiri bawah Studio dihilangkan. **Implementasi:** `components/InvitationStudio/InvitationDesigner.tsx` tidak lagi menetapkan status tersebut setelah undangan selesai dimuat dan hanya merender footer status jika ada pesan yang relevan. Peringatan template belum disimpan, progres unggah, hasil Simpan Desain, serta pesan kesalahan tetap tersedia ketika diperlukan; tidak ada perubahan mekanisme penyimpanan, data undangan, atau layout header/canvas. **Validasi:** perubahan source disimpan di GitHub (`e12d53d1`); build dan inspeksi browser belum dijalankan.


### 25 September 2026 — Pindahkan Kembalikan ke Default ke bawah Isi

**Permintaan owner:** tombol reset tema pada toolbar atas dipindah ke daftar alat di kiri, tepat di bawah **Isi**, memakai ikon panah putar yang telah digunakan dan label **Ulang dari awal**. **Implementasi:** `components/InvitationStudio/InvitationDesigner.tsx` menghilangkan tombol Kembalikan ke Default dari toolbar atas dan memasang `DesignerTool` aksi reset di bawah Isi tanpa memindahkan menu lain. `components/InvitationStudio/DesignerPanels.tsx` mendukung opsi `disabled` dan `title` pada tombol rail untuk mencegah reset saat undangan belum siap/sedang menyimpan; `studio.css` memperjelas disabled state. Fungsi `restoreDefaults` tetap seperti sebelumnya: hanya mereset warna/font/bagian, tidak menghapus isi, foto, musik atau objek. Ikon `RotateCcw` di toolbar preview tetap punya fungsi berbeda (ulang animasi Amplop) dan tidak diubah. `tests/invitation-asset-layers.test.mjs` ditambah pemeriksaan lokasi alat, label dan pembatasan interaksi. **Validasi:** perubahan tersimpan di GitHub; build Next.js serta pemeriksaan browser desktop/HP belum terverifikasi.


### 25 September 2026 — Pulihkan editan Studio saat refresh saja, buang setelah keluar

**Permintaan owner:** refresh halaman Studio tidak menghilangkan editan yang belum disimpan; setelah logout atau meninggalkan halaman lalu kembali, perubahan itu tidak boleh muncul lagi. **Implementasi:** `lib/templates/studio-refresh-draft.ts` memberi snapshot tervalidasi dengan versi, ID undangan, baseline state tersimpan di server, dan state sementara (design key termasuk aset/teks/section/foto, URL musik pilihan, hashtag, dress code). `components/InvitationStudio/InvitationDesigner.tsx` menyimpan perubahan belum tersimpan ke `sessionStorage` tab sendiri, memulihkan hanya ketika PerformanceNavigationTiming menunjukkan browser reload pada event dan baseline tersimpan yang identik, dan menghapus cache pada Save berhasil, klik link keluar, browser Back/Forward, atau saat halaman bfcache dipulihkan. Navigasi biasa, pindah undangan atau baseline server berubah otomatis menolak dan menghapus snapshot. Tidak mengubah API/auth, data/publikasi DB, master template atau desain publik; tidak membuat autosave server. `tests/studio-refresh-draft.test.mjs` menambahkan kasus refresh/cross-event/baseline berubah/navigasi/invalid payload dan kontrak pembersihan. **Validasi:** source sudah dikomit ke main (`ec4efed7`, `d0fb3d29`, `a9b28cf0`, `8bf975c4`); build Next.js dan uji manual reload/logout/Back/Forward di browser masih belum terverifikasi.
...
### 26 September 2026 — Pemulihan verifikasi email yang gagal terkirim

**Masalah:** pendaftaran membuat akun belum terverifikasi dan selalu mengembalikan sukses walau pengiriman email produksi gagal; percobaan daftar ulang hanya mendapat konflik email tanpa cara meminta tautan baru. Transport email yang melempar exception juga membuat respons tidak konsisten.
