...
- Gunakan preview terisolasi `/pintu-lab` untuk Pintu 1 agar existing navigation/orbit empat pintu pada landing utama `/` tidak berubah sebelum visual direview/di-approve owner. Eksperimen `/jiplak` tetap independen; jumlah pintu uji V2 adalah eksperimen historis, bukan instruksi mengganti empat pintu produksi. Patuhi reduced motion dan evaluasi desktop/mobile.
...
- `/pintu-lab` menjadi tempat rebuild Three.js; eksperimen CSS lama tetap di `/pintu-lab/css` sebagai arsip/pembanding dan `/jiplak` tetap eksperimen terpisah. **Landing utama `/` dan orbital empat pintu yang sekarang aktif tidak diubah** oleh eksperimen Pintu 1 sebelum approval visual. Engine harus lazy-load di route lab, memperhatikan viewport/reduced motion, membersihkan WebGL resources, dan menyediakan fallback saat WebGL tidak tersedia.
...
- Implementasikan pengujian interaksi transisi **setelah Pintu 1 disetujui visualnya**: Tahap 10 lighting menyiapkan titik terang di bukaan; Tahap 11–13 memverifikasi clipping/responsivitas/performa; Tahap 14 membuat komponen Pintu reusable; **Tahap 15** pada rancangan V2 historis menguji reuse tiga pintu di `/pintu-lab`. Ini bukan spesifikasi jumlah pintu produksi: integrasi ke landing, bila nanti secara eksplisit disetujui owner, harus mempertahankan keempat tujuan layanan dan kontrak transisi landing aktif. Landing utama `/` tidak diubah selama preview `/pintu-lab` belum disetujui.
