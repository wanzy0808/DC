...
**Kontrak menu Isi:** ikon **Isi** di rail kiri bertugas mengubah kata-kata undangan yang memang boleh dikustomisasi oleh pemilik event: salam/permohonan kehadiran (`greeting`), ucapan penutup (`closing`), serta doa/kutipan **hanya apabila template dan kategori event benar-benar menampilkan slotnya** (untuk implementasi Zen Atelier pernikahan: `zenQuote` pada bagian penutup). Field ditentukan oleh registry `lib/templates/editable-copy.ts` sesuai kemampuan renderer, dengan label/batas karakter terukur. Bila suatu template tidak memiliki slot doa/kutipan, **jangan tampilkan editor palsu yang tidak mengubah undangan**. Contoh awal Universal/Romantic Rose mendukung `greeting` dan `closing`; Zen Atelier pada kategori wedding juga mengizinkan `zenQuote`. Tambahan slot masa depan membutuhkan render nyata dan registrasi terlebih dahulu.
