...
### 7.2.3b Sebelas tema berbeda: lima dengan foto, enam tanpa foto (24 September 2026)

Setiap template READY harus punya komposisi nyata yang dapat dibedakan secara visual sebelum dan setelah membuka amplop, bukan hanya pergantian palette, font, stock photo dan border radius pada satu layout. Manifest tunggal `lib/templates/catalog.ts` menyatakan `usesPhotos: boolean`, `photoSlots`, nama dan preset. Katalog bawaan sejak 24 September 2026 memiliki **11 template (10 key existing dipertahankan + Zen Atelier), 5 dengan foto dan 6 tanpa foto**:
...
**Zen Atelier — pengelolaan aset (24 September 2026):** Kode ilustrasi disimpan pada `assets/templates/zen-atelier/`, bukan file statis `public/`. Ilustrasi vektor dalam bundle frontend dan turunan preview publik tetap dapat disalin dan **bukan** proteksi atas file master. File master beresolusi tinggi/lisensi harus tinggal di private storage di luar Git dan public web root dengan akses server-side yang sesuai; `.gitignore` saja bukan mekanisme keamanan dan tidak menyembunyikan riwayat repo public. Gambar asli dari shared chat Zen Atelier belum ditemukan di `main` ketika template dibuat sehingga ilustrasi vektor baru adalah interpretasi moodboard, bukan migrasi file asli. Media customer tetap memakai pipeline Sharp/WebP/InvitationAsset yang sudah berjalan; tidak ada perubahan API RSVP/Wishes/Gift atau database.
...
- Added ready theme `zen-atelier` as the eleventh built-in (5 photo + 6 no-photo). Existing keys, public routes, shared 15 toggles, real RSVP form, gift, honest Wishes empty-state and music engine retained. The theme supplies its own lazy-loaded envelope/cover, vector sakura/ensō/ink landscapes and no-photo story artwork, new palette/font preset and a lightweight public thumbnail.
