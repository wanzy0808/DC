...
**Baseline aktif adalah homepage `/` dengan empat pintu layanan dan komposisi yang sudah disetujui owner (§15.4.1b).** Pintu marketing tidak boleh diganti, dikurangi atau dirombak demi menghidupkan kembali eksperimen. Tidak ada kewajiban melanjutkan 15 tahap rekonstruksi `/pintu-lab` kecuali owner meminta proyek baru secara eksplisit. Catatan V1/V2 Three.js, target `pintu1.png`, screenshot, dan persetujuan per tahap adalah riwayat penelitian desain, **bukan instruksi baru untuk mengubah landing aktif**. Rincian dokumen `pintu3d.md` sebelumnya tetap tersedia melalui tautan arsip Git pada Appendix A. Jika eksperimen pintu dibuka kembali, buat scope baru berdasarkan kondisi source dan approval owner saat itu, pertahankan empat layanan dan perilaku landing yang sudah ada sampai owner menyetujui perubahan spesifik.

### 15.4.1b — Approved production landing baseline (21 September 2026)

Owner approved the landing composition formerly developed under `/pagecontoh`, which has since been moved into **`app/page.tsx` as the active production homepage**. The earlier `/pagecontoh` route/file is no longer present on the verified repository branch; do not restore a duplicate page merely to satisfy legacy documentation. Preserve the approved composition currently implemented on `/` without unsolicited redesign. This approval supersedes earlier instructions prohibiting promotion of the preview to `/` and applies only to this exact approved composition, **not** to the separate `/pintu-lab` GLB experiment.
