...
- Source visual/interaksi yang berlaku adalah `app/page.tsx`, `components/Landing/Pintu/SimpleDoorLab.tsx` dan `PortalTransition` yang dipakai aplikasi. Rencana lama berupa kamera terbang melewati ruang 3D laboratorium, integrasi tiga pintu, atau zoom-out seluruh halaman **tidak otomatis menjadi requirement untuk merombak implementasi final**. Ubah landing hanya atas instruksi owner yang spesifik; catatan eksperimen lama tersimpan di Git history.
...
| `Dashboard-redesign.md` | Jurnal tahapan desain Dashboard; bukan PRD produk aktif. | Ambil keputusan aktif dari §6 dan §15; jurnal tidak boleh menimpa PRD. |
