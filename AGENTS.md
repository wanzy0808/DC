...
- Explicit UI exception: **burger navigation buttons** may use a neutral white surface with black text and a subtle border in both themes. Login/Register are no longer part of this exception: account-entry forms follow the shared Rose auth system in `components/Auth/auth-styles.ts` (Rose outline/accent, theme-aware card/background, shared pill controls).
