...
- **Template gallery pill controls (22 September 2026):** On `/template-design` only, search inputs, filter chips, sort trigger and every sort option use consistent fully pill-rounded geometry with visible Rose outlines in both Light and Dark themes. The opened sort menu is a generously rounded capsule panel with Rose outline; do not leave it as a rectangular/native dropdown or mix `rounded-xl` options with `rounded-full` triggers. Reuse `components/Templates/gallery-control-styles.ts` for search, filters, sort trigger, dropdown panel/options and gallery CTA; do not reintroduce per-element shape/border/focus classes. Gallery CTA stays on the canonical `Button` primitive with a gallery-scoped pill shape as explicitly requested by the owner. This is a scoped exception to the generic compact-rectangle Button shape rule; do not globalize it to Navbar, Dashboard, landing, or customer invitation templates. Retain ID/EN, A–Z/Z–A, keyboard and mobile behavior.
