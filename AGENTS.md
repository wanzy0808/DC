...
- **Dashboard sidebar contrast:** the customer sidebar follows the page canvas, not a full Rose rail. Light mode uses a white sidebar with readable near-black default text/icons; hover/active navigation may use Rose with white text/icons. Dark mode uses the same near-black background as the Dashboard body; sidebar navigation buttons use Rose with near-black text/icons, with Supporting/Deep Rose reserved for hover/active states. Sidebar navigation copy should be slightly larger than dense utility copy (roughly 15–16px for main items). Transparent header controls must remain high-contrast in Dark mode and must not inherit black text from the filled Rose button treatment.
