...
- **Three featured invitations on /d-invitation (22 September 2026):** The marketing Template Collection shows at most 3 READY templates from the shared catalog, ranked by count of unique event Payments with status PAID and a Digital Invitation package against the event\u2019s CURRENT `Invitation.templateKey`. Never invent sales or label random results bestsellers. Fill empty slots with randomly selected ready themes, and randomize all three before any paid adoption or if the aggregate database is unavailable. Fetch only public template keys through `app/api/templates/featured/route.ts`, keep actual customer/order data private, and do not modify the full public `/template-design` or Studio catalog. The highlight phone card must resemble the existing Hero mockup: aspect ~9:19.5, rounded metallic bezel, side buttons, notch, real lazy-loaded invitation preview; do not change Hero/Pintu/landing. There is no immutable template purchase snapshot yet; do not claim historical per-template sales based on current assigned keys. Keep the Template Collection header/title/description and right-hand full-gallery CTA grouped toward the center of the content frame (header max-width approximately 980px, with responsive spacing), instead of pinning the copy and button to opposite frame edges. This constraint applies only to the collection header; preserve the three phone previews, hero composition and other marketing sections. The `Lihat semua template` / `Explore all templates` CTA uses normal text case and normal letter spacing, not forced uppercase/tracking. For this section, the underline/divider below the header spans the entire collection content width while the header copy and right CTA remain grouped in their centered max-980px inner wrapper; keep a visibly generous divider-to-smartphone gap (about 56px mobile, 64px desktop). The smartphone itself links to the preview, so do not render a redundant preview button below any phone card.
