...
- **Dashboard sidebar contrast:** the customer sidebar follows the page canvas, not a full Rose rail. Light mode uses a white sidebar with readable near-black default text/icons; Dark mode uses the same near-black background as the Dashboard body with white default text/icons. In both themes, navigation buttons remain neutral/transparent by default and use Rose with white text/icons for hover/active states; Deep Rose may mark the active state. Sidebar navigation copy should be slightly larger than dense utility copy (roughly 15–16px for main items). Dashboard header controls (burger, theme toggle, ID/EN, and account trigger) form one visual family. Light mode uses a white/neutral idle surface with a restrained Rose border and Rose foreground; Dark mode uses the near-black Dashboard surface with white foreground. All share the same compact rounded shape, border treatment, and Rose + white hover state. Selected language is indicated by border/underline emphasis rather than a separate filled color system. Account dropdown items keep the same neutral-first Dark Mode behavior.
