...
- **Pintu 3D staged visual work:** For the approved Pintu 1-only experiment, read `pintu3d.md` before editing geometry/assets/motion. Work first in isolated `/pintu-lab` and keep the entire hinged leaves, attached detail, and their backfaces moving as a unit while the frame stays fixed. Do not promote to `/` or replicate into three orbital doors until Pintu 1 passes owner visual review.
