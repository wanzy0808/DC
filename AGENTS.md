...
- **Dashboard sidebar contrast:** the customer sidebar is one Rose navigation rail. Light mode uses white text/icons; Dark mode uses near-black text/icons. Active/hover treatment is a restrained neutral overlay, not additional Rose card fills. Transparent header controls must remain high-contrast in Dark mode and must not inherit black text from the filled Rose button treatment.
