...
const heading = { fontFamily: "var(--font-dc-heading), Georgia, serif" };
...
function BotanicalSprig({ mirrored = false }: { mirrored?: boolean }) {
  return <div aria-hidden className={`absolute top-0 h-[290px] w-28 text-[#71826a] ${mirrored ? "-right-5 -scale-x-100 rotate-[-10deg]" : "-left-5 rotate-[-10deg]"}`}>
...
export default function InvitationThemeScenes({theme,names,date,cover,focus,stage,onOpen,onEditPhoto,preview}: SceneProps) {
  ...
  if (theme === "eternal-blossom") return <section className={`${center} bg-[#ffedf0] text-[#622a43]`} data-invitation-section={stage}>
    ...
    <Flower2 aria-hidden className="absolute -left-12 top-14 h-48 w-48 -rotate-[32deg] text-[#cd7390]/50" strokeWidth={0.6} />
    <Flower2 aria-hidden className="absolute -right-12 bottom-24 h-52 w-52 rotate-[20deg] text-[#c45c7e]/40" strokeWidth={0.6} />
    <p className={`${caption} relative mb-6 text-[#a45c75]`}>{isEnvelope ? "A love letter" : "Eternal Blossom"}</p>
    <div className="relative w-[min(70vw,290px)] rotate-[-4deg] rounded-t-full rounded-b-[100px] border-8 border-white bg-white p-2 shadow-[20px_20px_0_#dd9caf]">
      ...
      {isEnvelope && <span aria-hidden className="absolute -bottom-7 -right-7 grid h-20 w-20 place-items-center rounded-full border-4 border-white bg-[#a8486c] text-white shadow-xl"><Heart className="h-8 w-8" fill="currentColor" /></span>}
    </div>
    ...
    {isEnvelope && preview && <p className="mt-5 text-xs opacity-60">Pratinjau · data contoh</p>}
  </section>;

  if (theme === "modern-maroon") return <section className={`${center} bg-[#4b0f1e] text-[#ffe4dd]`} data-invitation-section={stage}>
    <div aria-hidden className="absolute left-0 top-0 h-full w-[20%] bg-[#d77e6d]" />
    <div aria-hidden className="absolute right-0 top-0 h-full w-[12%] bg-[#7d2030]" />
    <span aria-hidden className="absolute left-[13%] top-6 text-[110px] font-black leading-none text-[#f9b6a3]/20">M.</span>
    <p className={`${caption} relative mb-7 self-start text-[#f8af99]`}>{isEnvelope ? "Private invitation / 01" : content}</p>
    <div className="relative flex w-full max-w-[370px] items-start justify-center gap-3">
      <div className="relative h-[340px] w-[66%] -skew-y-[3deg] overflow-hidden border-4 border-[#e7a79a] shadow-[14px_14px_0_#7d2030]"><Portrait src={cover} focus={focus} alt="Foto utama undangan" /><Edit onClick={!isEnvelope?onEditPhoto:undefined}/></div>
      <p className="[writing-mode:vertical-rl] rotate-180 text-[10px] uppercase tracking-[.4em]">DC Organizer — Selected events</p>
    </div>
    <div className="relative mt-8 w-full max-w-[370px] border-t border-[#e7a79a]/55 pt-6 text-left">
      <Names className="text-3xl uppercase">{names}</Names><p className="mt-3 text-xs tracking-[.2em]">{date}</p>
    </div>
    {isEnvelope ? <Open dark onClick={onOpen}>OPEN INVITATION ↗</Open> : <Lines className="mt-9" />}
  </section>;

  if (theme === "garden-light") return <section className={`${center} bg-[#ecf0de] text-[#465c3a]`} data-invitation-section={stage}>
    <div aria-hidden className="absolute -left-24 top-20 h-72 w-72 rounded-full border-[18px] border-[#a6be92]/40" />
    <div aria-hidden className="absolute -right-28 bottom-20 h-80 w-80 rounded-full border-[18px] border-[#acc89b]/40" />
    <BotanicalSprig /><BotanicalSprig mirrored />
    <p className={`${caption} relative mt-6 text-[#68855d]`}>{isEnvelope ? "A garden invitation" : "In full bloom"}</p>
    <div className="relative mt-9 w-[min(73vw,290px)] rounded-t-[180px] rounded-b-[16px] border-[12px] border-[#f8faed] bg-[#e1ebd9] p-1 shadow-[0_20px_48px_#41593733]">
      ...
      <span aria-hidden className="absolute -bottom-7 left-1/2 grid h-14 w-14 -translate-x-1/2 place-items-center rounded-full border-4 border-[#ecf0de] bg-[#607c52] text-white"><Leaf className="h-6 w-6" /></span>
    </div>
  ...
  if (theme === "midnight-romance") return <section className={`${center} bg-[#080d20] text-[#f4e7d0]`} data-invitation-section={stage}>
    ...
    <Moon aria-hidden className="absolute right-7 top-14 h-20 w-20 text-[#e5d09e]/45" strokeWidth={0.6} />
    <p className={`${caption} relative mb-7 text-[#e6cda0]`}>{isEnvelope ? "Under the stars" : "Midnight Romance"}</p>
    <div className="relative h-[310px] w-[min(75vw,310px)] rounded-full border border-[#e1ca8f] p-2 shadow-[0_0_0_7px_#e1ca8f20,0_0_0_20px_#e1ca8f0a]">
      ...
      <Star aria-hidden className="absolute -right-5 top-8 h-9 w-9 text-[#e6cda0]" strokeWidth={0.7} />
    </div>
    <Names className="relative mt-10 text-3xl">{names}</Names>
    <p className="mt-3 text-xs uppercase tracking-[.3em] text-[#e5d5ac]">{date}</p>
    {isEnvelope ? <Open dark onClick={onOpen}>Buka Undangan ✧</Open> : <Lines className="mt-8"><Star className="h-4 w-4"/></Lines>}
  </section>;

  if (theme === "botanical-ivory") return <section className={`${center} bg-[#faf7e9] text-[#50634d]`} data-invitation-section={stage}>
    <BotanicalSprig /><BotanicalSprig mirrored />
    <div aria-hidden className="absolute left-6 top-7 h-20 w-20 rounded-tl-[70px] border-l border-t border-[#879a77]/50" />
    <p className={`${caption} relative mb-12 text-[#768968]`}>Botanical • Ivory</p>
    <div className="relative flex min-h-[370px] w-[min(77vw,300px)] flex-col items-center justify-center border border-[#8da080] bg-[#fffdf3] px-6 py-10 shadow-[0_22px_0_#dfddc9,0_30px_50px_#5265481c]">
      <span aria-hidden className="absolute inset-3 border border-[#c5d1b4]"/>
      <Leaf aria-hidden className="relative mb-8 h-12 w-12 -rotate-45 text-[#748c6b]" strokeWidth={0.65}/>
      <p className={`${caption} relative mb-5 text-[#809179]`}>{content}</p>
  ...
  if (theme === "classic-pearl") return <section className={`${center} bg-[#f8f6ef] text-[#37352f]`} data-invitation-section={stage}>
    <span aria-hidden className="absolute inset-5 border border-[#b4a88c]" />
    <span aria-hidden className="absolute inset-8 border border-[#d8ceba]" />
    <p className={`${caption} relative mt-14 text-[#8a7c62]`}>{isEnvelope ? "The private invitation" : "Classic Pearl"}</p>
    <div className="relative mt-10 flex h-[280px] w-[min(66vw,265px)] items-center justify-center rounded-full border border-[#baa989]">
      <div aria-hidden className="absolute inset-3 rounded-full border border-[#c9b99b]" />
      <Gem aria-hidden className="absolute -top-5 h-10 w-10 bg-[#f8f6ef] p-2 text-[#917f5c]" strokeWidth={0.8} />
  ...
  if (theme === "golden-art-deco") return <section className={`${center} bg-[#191b17] text-[#e4c888]`} data-invitation-section={stage}>
    <div aria-hidden className="absolute inset-4 border border-[#ba9a55]/65" />
    <div aria-hidden className="absolute inset-8 border border-[#ba9a55]/30" />
    <div aria-hidden className="absolute left-1/2 top-0 h-52 w-52 -translate-x-1/2 rotate-45 border border-[#bd9e59]/70" />
    <div aria-hidden className="absolute left-1/2 top-[-80px] h-52 w-52 -translate-x-1/2 rotate-45 border border-[#bd9e59]/40" />
    <div aria-hidden className="absolute bottom-[-105px] left-1/2 h-64 w-64 -translate-x-1/2 rotate-45 border border-[#bd9e59]/40" />
    <p className={`${caption} relative mt-20`}>{isEnvelope ? "You are cordially invited" : "A gilded celebration"}</p>
    <div aria-hidden className="relative mt-10 flex items-center gap-2">{[0,1,2,3,4].map(i=><span key={i} className="h-8 w-4 border-x border-t border-[#bf9c4b]" style={{height:`${33+Math.abs(2-i)*19}px`}}/>)}</div>
    <Names className="relative mt-14 max-w-xs text-3xl uppercase tracking-[.09em]">{names}</Names>
    <div aria-hidden className="relative mt-10 flex gap-3"><span className="h-14 w-px -rotate-45 bg-[#bf9c4b]"/><Gem className="h-7 w-7"/><span className="h-14 w-px rotate-45 bg-[#bf9c4b]"/></div>
  ...
  if (theme === "paper-cut-botanical") return <section className={`${center} bg-[#e9ead7] text-[#435e45]`} data-invitation-section={stage}>
    <div aria-hidden className="absolute -left-20 -top-10 h-[400px] w-64 rotate-[-32deg] rounded-full border-[55px] border-[#aebf96] bg-[#dce1c2] shadow-[15px_15px_0_#d1dcad]" />
    <div aria-hidden className="absolute -right-20 bottom-[-95px] h-[440px] w-72 rotate-[23deg] rounded-full border-[50px] border-[#9aaf88] bg-[#c6d0aa] shadow-[-15px_-15px_0_#d4dab4]" />
    <Leaf aria-hidden className="absolute -left-3 top-12 h-40 w-40 rotate-[-30deg] fill-[#b2c2a0] text-[#819975]" strokeWidth={0.7}/>
    <Leaf aria-hidden className="absolute -right-4 bottom-20 h-44 w-44 rotate-[170deg] fill-[#a4b998] text-[#76926e]" strokeWidth={0.7}/>
    <p className={`${caption} relative mb-10 text-[#687b57]`}>Handcrafted in paper</p>
    <div className="relative flex min-h-[350px] w-[min(77vw,300px)] flex-col items-center justify-center rounded-t-[155px] border-[9px] border-[#fdfcf1] bg-[#f7f6e9] px-7 py-9 shadow-[12px_16px_0_#aabf92]">
      <Sun aria-hidden className="mb-7 h-10 w-10 text-[#93a97c]" strokeWidth={0.8}/>
  ...
  if (theme === "celestial-ink") return <section className={`${center} bg-[#101b32] text-[#c9e2f0]`} data-invitation-section={stage}>
    ...
    <div aria-hidden className="absolute left-1/2 top-[18%] h-[420px] w-[420px] -translate-x-1/2 rounded-full border border-[#a4c0e1]/40"/>
    <div aria-hidden className="absolute left-1/2 top-[23%] h-[340px] w-[340px] -translate-x-1/2 rounded-full border border-[#a4c0e1]/55"/>
    <div aria-hidden className="absolute left-1/2 top-[29%] h-[260px] w-[260px] -translate-x-1/2 rounded-full border border-[#a4c0e1]/50"/>
    <Moon aria-hidden className="relative mt-14 h-16 w-16 text-[#b8cfea]" strokeWidth={0.65}/>
    <p className={`${caption} relative mt-8 text-[#a3c8e5]`}>Written in the stars</p>
