...
type Props = { order: { id: string; invoiceNumber: string; packageKey: string; packageName: string; amount: number; status: string; proofUrl: string | null; note: string | null; reportedAt: string | null; createdAt: string; invitation: { title: string; groomName: string; brideName: string } }; bank: { name: string; account: string; holder: string } };
...
export default function CheckoutClient({ order, bank }: Props) {
  ...
  const [reportedAt, setReportedAt] = useState(order.reportedAt);
  ...
  async function submitProof() {
    ...
    try {
      ...
      setReportedAt(data.order?.reportedAt ? String(data.order.reportedAt) : new Date().toISOString());
  ...
  async function reportPaid() {
    setSaving(true); setMessage("");
    try {
      const response = await fetch(`/api/orders/${order.id}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action: "REPORT_PAID" }) });
      const data = await response.json();
      if (!response.ok) throw new Error(data.error ?? "Laporan pembayaran belum dapat dikirim.");
      setReportedAt(data.order?.reportedAt ? String(data.order.reportedAt) : new Date().toISOString());
      setMessage("Pembayaran sudah dilaporkan. Tim kami akan memverifikasinya.");
    } catch (error) { setMessage(error instanceof Error ? error.message : "Laporan pembayaran belum dapat dikirim."); } finally { setSaving(false); }
  }
