...
export default function Footer() {
  ...
  if (pathname === "/") {
    return <footer className="absolute bottom-0 left-0 w-full py-3 text-center text-[10px] md:text-xs opacity-50 font-mono tracking-wider z-20 bg-transparent text-[var(--foreground)] border-none pointer-events-none">© {new Date().getFullYear()} DC Organizer. All Rights Reserved.</footer>;
  }
  ...
  return <footer className={`w-full border-none bg-transparent transition-colors duration-500 z-20 ${isDarkMode ? "text-white" : "text-[#1A1A1A]"}`}>
    <div className="w-full max-w-[75%] mx-auto px-6 py-12 md:py-16 space-y-12">
      <div className="grid grid-cols-1 md:grid-cols-12 gap-8 items-start justify-between">
        <div className="md:col-span-6 space-y-4"><Link href="/" className="inline-block"><div className={`font-serif text-3xl font-bold tracking-[0.2em] ${isDarkMode ? "text-[#F4C2C7]" : "text-[#8C4A56]"}`}>D C</div><span className="text-[9px] font-sans tracking-[0.25em] opacity-60 uppercase block">ORGANIZER</span></Link><p className="text-xs max-w-sm leading-relaxed opacity-70 font-light">DC Organizer adalah platform digitalisasi pernikahan terpadu yang membuat pernikahanmu lebih efisien, berkesan, dan elegan.</p><div className="pt-2 text-xs font-mono opacity-80 space-y-1"><p>Layanan Pelanggan:</p><p className={`font-semibold ${isDarkMode ? "text-[#E8B4B8]" : "text-[#8C4A56]"}`}>halo@dcwedding.co | +62 812-3456-7890</p></div></div>
        <div className="md:col-span-6 md:text-right space-y-3"><h4 className="text-xs font-mono uppercase tracking-widest opacity-80">Metode Pembayaran</h4><div className="flex md:justify-end items-center"><div className={`p-3 rounded-2xl border backdrop-blur-md inline-flex items-center justify-center ${isDarkMode ? "bg-white/5 border-white/10 shadow-lg" : "bg-white border-[#8C4A56]/10 shadow-md"}`}><Image src="/bca.webp" alt="Bank BCA" width={75} height={25} className="object-contain h-6 w-auto" /></div></div></div>
      </div>
      <div className="grid grid-cols-1 md:grid-cols-4 gap-8 pt-8"><div className="space-y-3"><h5 className={`text-xs font-serif font-bold ${isDarkMode ? "text-[#F4C2C7]" : "text-[#8C4A56]"}`}>Produk</h5><ul className="space-y-2 text-xs opacity-70 font-light"><li><Link href="/D-invitation">Undangan Digital</Link></li><li><Link href="/guestbook">Buku Tamu QR</Link></li><li><Link href="/wedding-organizer">WO & Planning</Link></li></ul></div><div className="space-y-3"><h5 className={`text-xs font-serif font-bold ${isDarkMode ? "text-[#F4C2C7]" : "text-[#8C4A56]"}`}>Bantuan</h5><ul className="space-y-2 text-xs opacity-70 font-light"><li>FAQ</li><li>Syarat & Ketentuan</li><li>Kebijakan Privasi</li></ul></div><div className="space-y-3"><h5 className={`text-xs font-serif font-bold ${isDarkMode ? "text-[#F4C2C7]" : "text-[#8C4A56]"}`}>Resources</h5><ul className="space-y-2 text-xs opacity-70 font-light"><li>Katalog Template</li><li>Blog & Artikel</li></ul></div><div className="space-y-3"><h5 className={`text-xs font-serif font-bold ${isDarkMode ? "text-[#F4C2C7]" : "text-[#8C4A56]"}`}>Ikuti Kami</h5><div className="flex items-center gap-3">{[{icon:InstagramIcon,href:"#"},{icon:TiktokIcon,href:"#"},{icon:FacebookIcon,href:"#"},{icon:YoutubeIcon,href:"#"}].map((item,index)=>{const IconComponent=item.icon;return <a key={index} href={item.href} className={`p-2.5 rounded-full border transition-all duration-300 hover:scale-110 ${isDarkMode ? "bg-[#E8B4B8]/15 border-[#E8B4B8]/30 text-[#F4C2C7] hover:bg-[#E8B4B8] hover:text-[#0F0E11]" : "bg-[#8C4A56]/10 border-[#8C4A56]/20 text-[#8C4A56] hover:bg-[#8C4A56] hover:text-white"}`}><IconComponent className="w-4 h-4" /></a>;})}</div></div></div>
