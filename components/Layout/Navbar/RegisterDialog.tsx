...
export default function RegisterDialog({ next: providedNext, onSwitchToLogin, onRegistered }: RegisterDialogProps) {
  ...
  return (
    <DialogContent overlayClassName="z-[100] bg-[#2a1620]/25 backdrop-blur-[2px] dark:bg-black/45" className={`${authCardClass} z-[101] max-h-[calc(100dvh-32px)] w-[min(94vw,490px)] gap-0 overflow-y-auto overscroll-contain p-6 sm:max-h-[min(88dvh,800px)] sm:max-w-[490px] sm:p-8`}>
        ...
        <div className={authChoiceBoxClass}>
          <label className="flex cursor-pointer items-start gap-3">
            ...
            <span className="leading-relaxed">
              {t.terms} <Link href="/terms-and-conditions" target="_blank" rel="noopener noreferrer" className="font-medium text-primary underline decoration-primary/40 underline-offset-4 transition-colors hover:decoration-primary focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-primary" onClick={(event) => event.stopPropagation()}>{t.termsName}</Link> {t.and} <Link href="/privacy-policy" target="_blank" rel="noopener noreferrer" className="font-medium text-primary underline decoration-primary/40 underline-offset-4 transition-colors hover:decoration-primary focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-primary" onClick={(event) => event.stopPropagation()}>{t.privacy}</Link>.
            </span>
          </label>
          <label className="flex cursor-pointer items-start gap-3">
            <input
