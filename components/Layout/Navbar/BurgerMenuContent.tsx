      ...
      <div>
        ...
        <nav className="space-y-3 font-[family-name:var(--font-dc-sans)] text-sm">
          <SheetClose nativeButton={false} render={<Link href="/login" className={itemClass} />}>
            <span className="flex items-center gap-3"><LogIn className={`h-5 w-5 ${accent}`} />Masuk</span>
            <span className="font-mono text-xs opacity-50">→</span>
          </SheetClose>

          <Dialog open={isRegisterOpen} onOpenChange={setIsRegisterOpen}>
            <DialogTrigger className={itemClass}>
              <span className="flex items-center gap-3"><UserPlus className={`h-5 w-5 ${accent}`} />Daftar</span>
              <span className="font-mono text-xs opacity-50">+</span>
            </DialogTrigger>
          ...
          <div className={`overflow-hidden rounded-xl border ${border}`}>
            <button type="button" onClick={() => setIsProductOpen((open) => !open)} className={`flex w-full items-center justify-between p-3.5 text-left ${hover}`}>
              <span className="flex items-center gap-3"><Package className={`h-5 w-5 ${accent}`} />Produk</span>
            ...
            {isProductOpen && (
              <div className={`space-y-1 border-t ${border} p-2`}>
                {products.map(({ href, label, icon: Icon }) => (
                  <SheetClose key={href} nativeButton={false} render={<Link href={href} className={`flex items-center justify-between rounded-lg px-4 py-2.5 text-sm ${hover} transition`} />}>
          ...
          <SheetClose nativeButton={false} render={<Link href="/packages" className={itemClass} />}>
            <span className="flex items-center gap-3"><Package className={`h-5 w-5 ${accent}`} />Package</span>
            <span className="font-mono text-xs opacity-50">→</span>
          </SheetClose>

          <SheetClose nativeButton={false} render={<Link href="/template-design" className={itemClass} />}>
            <span className="flex items-center gap-3"><LayoutTemplate className={`h-5 w-5 ${accent}`} />Template</span>
            <span className="font-mono text-xs opacity-50">→</span>
          </SheetClose>

          <SheetClose nativeButton={false} render={<Link href="/help" className={itemClass} />}>
            <span className="flex items-center gap-3"><CircleHelp className={`h-5 w-5 ${accent}`} />Bantuan</span>
            <span className="font-mono text-xs opacity-50">→</span>
          </SheetClose>
        </nav>
      </div>
