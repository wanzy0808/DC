...
export default function BurgerMenuContent({ onClose }: { onClose: () => void }) {
  ...
  const items = [
    ...
    { href: "/template-design", label: nav.templates },
    { href: "/help", label: nav.help },
  ];
  const services = [
    { href: "/event-planner", label: nav.planner },
    { href: "/d-invitation", label: nav.invitation },
    { href: "/guestbook", label: nav.guestbook },
  ];
  const itemClass = "flex min-h-10 w-full items-center !rounded-[18px] border border-primary/30 bg-background/35 px-4 py-2.5 text-left font-[family-name:var(--font-dc-heading)] text-sm text-primary shadow-none transition-[background-color,border-color,transform] duration-200 hover:-translate-y-0.5 hover:border-primary/65 hover:bg-primary/10 hover:text-primary dark:text-primary dark:hover:text-primary";
  ...
  return (
    <nav aria-label={nav.navigation} className="space-y-2">
      <motion.div {...reveal(0)}>
        <Link href="/login" onClick={onClose} aria-current={pathname === "/login" ? "page" : undefined} className={itemClass}>{nav.login}</Link>
      </motion.div>
      <motion.div {...reveal(1)}>
        <Dialog open={registerOpen} onOpenChange={setRegisterOpen}>
          <DialogTrigger render={<Button className={itemClass} />}>{nav.register}</DialogTrigger>
      ...
      <motion.div {...reveal(5)}>
        <button type="button" aria-expanded={servicesOpen} onClick={() => setServicesOpen((open) => !open)} className={itemClass}>
          <span>{nav.services}</span><ChevronDown className={`ml-auto size-4 transition-transform duration-200 ${servicesOpen ? "rotate-180" : ""}`} />
        </button>
      </motion.div>
