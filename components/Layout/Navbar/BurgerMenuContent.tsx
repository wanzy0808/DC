...
export default function BurgerMenuContent({ isDarkMode }: BurgerMenuContentProps) {
  ...
  const itemClass = "flex w-full items-center justify-between rounded-2xl border border-[var(--border)] bg-[var(--card)]/55 p-4 text-left transition hover:-translate-y-0.5 hover:border-[color-mix(in_srgb,var(--primary)_28%,var(--border))] hover:bg-[var(--muted)]";
  void isDarkMode;

  return (
    <SheetContent side="right" className="flex w-full flex-col justify-between border-l border-[var(--border)] bg-[var(--background)] p-6 text-[var(--foreground)] sm:w-[430px]">
      <div>
        <SheetHeader className="mb-8 text-left">
          ...
          <p className="mt-2 text-sm leading-6 text-[var(--muted-foreground)]">Pilih workspace atau layanan yang paling sesuai dengan tahap persiapanmu.</p>
        </SheetHeader>

        <nav className="space-y-3 font-[family-name:var(--font-dc-sans)] text-sm">
          <SheetClose nativeButton={false} render={<Link href="/login" className={itemClass} />}>
            <span className="flex items-center gap-3"><LogIn className="h-5 w-5 text-[var(--primary)]" />Masuk ke workspace</span><ArrowUpRight className="h-4 w-4 opacity-50" />
          </SheetClose>
          <Dialog open={isRegisterOpen} onOpenChange={setIsRegisterOpen}>
            <DialogTrigger className={itemClass}><span className="flex items-center gap-3"><UserPlus className="h-5 w-5 text-[var(--primary)]" />Buat akun baru</span><span className="font-[family-name:var(--font-dc-mono)] text-xs text-[var(--primary)]">JOIN</span></DialogTrigger>
          ...
          <div className="overflow-hidden rounded-2xl border border-[var(--border)] bg-[var(--card)]/40">
            <button type="button" onClick={() => setIsProductOpen((open) => !open)} className="flex w-full items-center justify-between p-4 text-left transition hover:bg-[var(--muted)]">
              <span className="flex items-center gap-3"><Package className="h-5 w-5 text-[var(--primary)]" />Layanan</span>
              <ChevronDown className={`h-4 w-4 transition-transform ${isProductOpen ? "rotate-180" : ""}`} />
            </button>
            {isProductOpen && <div className="space-y-1 border-t border-[var(--border)] p-2">
              {products.map(({ href, label, icon: Icon, description }) => <SheetClose key={href} nativeButton={false} render={<Link href={href} className="group flex items-center justify-between rounded-xl p-3 transition hover:bg-[var(--muted)]" />}><span className="flex items-center gap-3"><span className="flex h-9 w-9 items-center justify-center rounded-xl bg-[var(--secondary)]/55 text-[var(--primary)]"><Icon className="h-4 w-4" /></span><span><span className="block font-semibold">{label}</span><span className="block text-xs text-[var(--muted-foreground)]">{description}</span></span></span><ArrowUpRight className="h-4 w-4 opacity-30 transition group-hover:-translate-y-0.5 group-hover:translate-x-0.5 group-hover:opacity-100" /></SheetClose>)}
            </div>}
          </div>
      ...
      <div className="flex items-center justify-between border-t border-[var(--border)] pt-6 font-[family-name:var(--font-dc-mono)] text-[10px] uppercase tracking-[0.16em] text-[var(--muted-foreground)]"><span>DC Organizer</span><span>Workspace 1.0</span></div>
    </SheetContent>
  );
}
