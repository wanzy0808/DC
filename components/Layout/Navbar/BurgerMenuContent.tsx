...
export default function BurgerMenuContent({
  ...
  const products = [
    {
      ...
      label: nav.planner,
      icon: CalendarCheck,
      description: nav.plannerDescription,
    },
    {
      href: "/d-invitation",
      label: nav.invitation,
      icon: LayoutTemplate,
      description: nav.invitationDescription,
    },
    {
      href: "/guestbook",
      label: nav.guestbook,
      icon: BookOpen,
      description: nav.guestbookDescription,
    },
  ];
  ...
  return (
    <SheetContent
      side="right"
      className="flex w-[min(92vw,430px)] flex-col overflow-hidden border-l border-[var(--border)] bg-[var(--background)]/95 p-0 text-[var(--foreground)] shadow-2xl backdrop-blur-xl"
    >
      <motion.div
      ...
      >
        ...
        <nav className="font-[family-name:var(--font-dc-body)] text-sm">
          <div className="border-t border-[var(--border)]">
            <SheetClose
              nativeButton={false}
              render={<Link href={menuItems[0].href} className={itemClass} />}
            >
              <span className="flex items-center gap-4">
                <span className="font-[family-name:var(--font-dc-mono)] text-[10px] tracking-[0.16em] text-[var(--muted-foreground)]">
                  01
                </span>
                <LogIn className="h-[18px] w-[18px] text-[var(--primary)]" />
                <span>{nav.login}</span>
              </span>
            ...
            <Dialog open={isRegisterOpen} onOpenChange={setIsRegisterOpen}>
              <DialogTrigger className={itemClass}>
                ...
                <span className="font-[family-name:var(--font-dc-mono)] text-[10px] tracking-[0.16em] text-[var(--primary)]">
                  {nav.join}
                </span>
              </DialogTrigger>
              <RegisterDialog
            ...
            <div className="border-b border-[var(--border)]">
              ...
              >
                <span className="flex items-center gap-4">
                  ...
                  <Package className="h-[18px] w-[18px] text-[var(--primary)]" />
              ...
              <AnimatePresence initial={false}>
                {isProductOpen && (
                  ...
                  >
                    {products.map(
                      ({ href, label, icon: Icon, description }, index) => (
                        <motion.div
                          key={href}
                          initial={
                        ...
                        >
                          ...
                          >
                            <span className="flex min-w-0 items-center gap-3">
                              <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-[var(--secondary)] text-[var(--secondary-foreground)]">
                                <Icon className="h-4 w-4" />
                              </span>
                              <span className="min-w-0">
                                <span className="block font-medium">{label}</span>
                                <span className="block truncate font-[family-name:var(--font-dc-body)] text-xs text-[var(--muted-foreground)]">
                                  {description}
                                </span>
                              </span>
                            </span>
