...
export default function DashboardFeatureGuard({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    function onClick(event: MouseEvent) {
      if (access.guestbook) return;
      const target = event.target as HTMLElement | null;
      const button = target?.closest("button");
      if (!button) return;
      const text = button.textContent?.replace(/\s+/g, " ").trim().toLowerCase() ?? "";
      const locked = text.includes("tamu undangan") || text.includes("kelola tamu");
      if (locked) {
      ...
      {upgradeOpen && (
        <div className="fixed inset-0 z-[110] grid place-items-center bg-black/40 p-4 backdrop-blur-sm" onMouseDown={() => setUpgradeOpen(false)}>
          <div className="w-full max-w-md rounded-3xl border border-black/10 bg-[#fffdfa] p-7 shadow-2xl" onMouseDown={(event) => event.stopPropagation()}>
            ...
            <h2 className="mt-2 font-serif text-2xl">Kelola tamu dengan lebih lengkap</h2>
            <p className="mt-2 text-sm leading-6 text-black/50">Daftar tamu, nomor meja, kursi, Usher App, QR check-in, dan realtime attendance termasuk dalam Guestbook Digital.</p>
            <Link href="/packages" onClick={() => setUpgradeOpen(false)} className="mt-6 inline-flex w-full items-center justify-center rounded-xl bg-[#E60087] px-5 py-3 text-xs font-medium text-white">Lihat paket Guestbook Digital</Link>
          </div>
        </div>
      )}
    </>
