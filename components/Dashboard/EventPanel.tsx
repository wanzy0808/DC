...
export default function EventPanel({ accent, button, onSaved }: Props) {
  ...
  async function load(nextType: InvitationType) {
    ...
    try {
      ...
      setForm({ title: invitation.title || "", groomName: invitation.groomName || "", brideName: invitation.brideName || "", venue: invitation.venue || "", address: invitation.address || "", mapUrl: invitation.mapUrl || "", timezone: invitation.timezone || "Asia/Jakarta", eventDate: invitation.eventDate ? invitation.eventDate.slice(0, 10) : "", ceremonyTime: invitation.ceremonyTime || "", receptionTime: invitation.receptionTime || "", description: invitation.description || "", eventNotes: invitation.eventNotes || "" });
...
function Field({ label, value, onChange, placeholder = "", type = "text" }: { label: string; value: string; onChange: (value: string) => void; placeholder?: string; type?: string }) { return <label className="block"><span className="mb-1.5 block font-[family-name:var(--font-fauna)] text-xs font-semibold">{label}</span><Input type={type} value={value} onChange={(event) => onChange(event.target.value)} placeholder={placeholder} className="border-[#d8cbc2] bg-white font-[family-name:var(--font-fauna)] dark:border-white/10 dark:bg-black/20" /></label>; }
