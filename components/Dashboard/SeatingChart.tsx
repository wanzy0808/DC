...
type Props = { guests: Guest[]; tables: Table[]; accent: string; onAssigned: (guestId: string, tableId: string, seatNumber: number) => Promise<void>; onRefresh?: () => Promise<void> };
...
export default function SeatingChart({ guests, tables, accent, onAssigned, onRefresh }: Props) {
  ...
  async function generateTables(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (tables.length > 0) {
      setMessage("Denah sudah memiliki meja. Tambahkan meja melalui pengaturan meja yang sudah tersimpan.");
      return;
    }
    setGenerating(true);
    setMessage("");
    try {
      for (let index = 1; index <= count; index += 1) {
        const response = await fetch("/api/wedding-tables", {
          ...
          body: JSON.stringify({ name: `Meja ${index}`, capacity, shape: "ROUND" }),
        });
  ...
  async function assignFromDrop(event: React.DragEvent<HTMLDivElement>) {
    event.preventDefault();
    if (!draggedGuestId || !tables.length) return;
    ...
    tables.forEach((table, index) => {
      const center = tablePoint(index, tables.length);
      const seat = findSeat(point, table, center, occupiedByTable.get(table.id) ?? new Set());
    ...
    <div className="mt-6 grid gap-5 lg:grid-cols-[260px_minmax(0,1fr)]">
      <div className="rounded-2xl border border-[#d8cbc2] bg-[#fffaf6] p-4 dark:border-white/10 dark:bg-black/20">
        ...
        <form onSubmit={generateTables} className="mt-3 space-y-2 border-b border-[#d8cbc2] pb-4 dark:border-white/10">
          ...
          <button type="submit" disabled={generating || tables.length > 0} className="h-10 w-full rounded-xl bg-[#7A1C25] px-3 font-[family-name:var(--font-fauna)] text-xs font-semibold text-white transition hover:bg-[#5E141C] disabled:opacity-50">{generating ? "Membuat denah..." : tables.length ? `${tables.length} meja tersimpan` : "Buat Denah"}</button>
        </form>
        <p className="mt-3 font-[family-name:var(--font-fauna)] text-xs text-[#5A4545] dark:text-white/70">Masukkan jumlah meja dan kapasitas bangku. Setelah dibuat, grafik meja dan kursi mengikuti data database.</p>
        <p className={`mt-3 font-[family-name:var(--font-dm-mono)] text-[10px] font-semibold uppercase tracking-wider ${accent}`}>{tables.length} meja · {tables.reduce((sum, table) => sum + table.capacity, 0)} bangku</p>
      ...
      <div className="min-w-0 overflow-hidden rounded-2xl border border-[#d8cbc2] bg-[#fffaf6] p-2 dark:border-white/10 dark:bg-black/20" onDragOver={(event) => event.preventDefault()} onDrop={assignFromDrop}>
        <div className="overflow-auto"><Stage width={STAGE_WIDTH} height={STAGE_HEIGHT}>
          <Layer><Rect x={0} y={0} width={STAGE_WIDTH} height={STAGE_HEIGHT} fill="#F3EDE6" cornerRadius={18} listening={false} />
            {tables.map((table, tableIndex) => { const center = tablePoint(tableIndex, tables.length); return <Group key={table.id}>
            ...
            {!tables.length && <Text x={80} y={285} width={940} align="center" text="Masukkan jumlah meja dan bangku di panel kiri untuk membuat denah." fontSize={16} fontStyle="bold" fill="#5A4545" />}
          </Layer>
        </Stage></div>
