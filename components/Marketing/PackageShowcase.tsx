        ...
        {packages.map((item, index) => {
          ...
          return (
            <article key={item.key} className={`relative flex h-full flex-col rounded-3xl border p-6 md:p-7 ${featured ? "border-[var(--primary)] bg-[var(--primary)]/[0.07] shadow-[0_20px_60px_rgba(122,28,37,0.12)]" : "border-[var(--border)] bg-[var(--card)]/75"}`}>
              ...
              <Link href="/packages" className={`${buttonVariants({ variant: featured ? "default" : "outline" })} mt-7 w-full rounded-full px-5 py-3 text-xs font-semibold uppercase tracking-[0.16em]`}>{chooseLabel} <ArrowRight className="h-3.5 w-3.5" /></Link>
            </article>
          );
        })}
      </div>
