...
export default function RsvpForm({
  ...
  const [ticketGuest, setTicketGuest] = useState<{
    id: string;
    name: string;
    phone?: string | null;
    plusOnes: number;
      ...
      {ticketGuest && qrToken ? (
        <div className="text-center">
          ...
          <p className="mt-2 font-[var(--font-fauna)] text-sm text-[#5f4a4a] dark:text-white/65">
            Simpan Digital Ticket ini. QR di bawah adalah tiket unik untuk
            check-in hari H.
          </p>
          ...
          <p className="mt-4 font-mono text-[10px] text-[#6c5a5a] dark:text-white/50">
            Tiket tetap dapat ditunjukkan langsung dari ponsel saat tiba di
            venue.
          </p>
        </div>
      ) : (
            ...
            <div className="flex gap-5 text-sm">
              <label className="flex items-center gap-2">
                <input
                  ...
                  onChange={(e) =>
                    setForm({ ...form, plusOnes: e.target.value })
                  }
                />
                Ya
              </label>
