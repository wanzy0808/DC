    ...
    <section className="mx-auto max-w-xl border border-[#9b5b51]/20 bg-[#f3ede6] p-6 text-left shadow-sm dark:border-white/10 dark:bg-[#151116]">
      {ticketGuest && qrToken ? (
        <div className="text-center">
          ...
          <p className="mt-3 font-mono text-[10px] font-semibold uppercase tracking-[0.25em] text-[#7A1C25] dark:text-[#E8A5AE]">Smart RSVP berhasil</p>
          <h2 className="mt-2 font-[var(--font-cinzel)] text-3xl">Terima kasih, {ticketGuest.name}.</h2>
          <p className="mt-2 font-[var(--font-fauna)] text-sm text-[#5f4a4a] dark:text-white/65">Simpan Digital Ticket ini. QR di bawah adalah tiket unik untuk check-in hari H.</p>
          ...
          <div className="mt-5 grid gap-2 sm:grid-cols-2">
            <a href={ticketUrl} target="_blank" rel="noreferrer" className="inline-flex items-center justify-center gap-2 rounded-xl bg-[#7A1C25] px-4 py-3 font-[var(--font-fauna)] text-xs font-medium text-white"><QrCode className="h-4 w-4" />Simpan QR</a>
            <Button type="button" onClick={downloadTicket} variant="outline" className="h-auto rounded-xl border-[#7A1C25]/20 py-3 font-[var(--font-fauna)] text-xs"><Download className="h-4 w-4" />Download Ticket</Button>
          </div>
          <a href={calendarUrl} target="_blank" rel="noreferrer" className="mt-2 inline-flex w-full items-center justify-center gap-2 rounded-xl border border-[#7A1C25]/20 px-4 py-3 font-[var(--font-fauna)] text-xs text-[#7A1C25] dark:text-[#E8A5AE]"><CalendarPlus className="h-4 w-4" />Tambah ke Google Calendar</a>
          <p className="mt-4 font-mono text-[10px] text-[#6c5a5a] dark:text-white/50">Tiket tetap dapat ditunjukkan langsung dari ponsel saat tiba di venue.</p>
        </div>
      ) : (
        <form onSubmit={submit} className="space-y-4 font-[var(--font-fauna)]">
          <div><p className="font-mono text-[10px] uppercase tracking-[0.25em] text-[#9b5b51]">Smart RSVP</p><h2 className="mt-2 font-[var(--font-cinzel)] text-2xl">Konfirmasi Kehadiran</h2>{guestName && <p className="mt-1 text-sm opacity-70">Untuk: {guestName}</p>}</div>
          ...
          <Button disabled={submitting} className="rounded-xl bg-[#7A1C25] px-5 py-3 font-[var(--font-fauna)] text-xs text-white hover:bg-[#5E141C]">{submitting ? "Menyimpan..." : "Konfirmasi Kehadiran"}</Button>
