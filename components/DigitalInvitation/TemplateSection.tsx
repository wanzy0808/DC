...
const featuredTemplates: Template[] = [
  {
    key: "featured-eternal-blossom",
    name: "Eternal Blossom",
    theme: "Floral editorial",
    image: "/hp-digital.png",
  },
  {
    key: "featured-celestial-night",
    name: "Celestial Night",
    theme: "Dark gold evening",
    image: "/bca.webp",
  },
  {
    key: "featured-royal-velvet",
    name: "Royal Velvet",
...
export default function TemplateCollection() {
  ...
  const copy =
    locale === "en"
      ? {
          ...
          title: "Choose a visual direction that fits the event.",
          description:
            "Start from a template, preview the result, and use the same Studio flow whether you are preparing a wedding, anniversary, baby shower, birthday, or another celebration.",
          all: "See more templates",
          preview: "Preview",
          invitation: "Digital invitation",
          close: "Close preview",
          choose: "Choose this template",
          custom: "Designer template",
        }
      : {
          eyebrow: "Koleksi Template",
          title: "Pilih visual yang paling cocok dengan suasana acaramu.",
          description:
            "Mulai dari template, lihat gambaran hasilnya, lalu gunakan alur Studio yang sama untuk wedding, anniversary, baby shower, ulang tahun, atau perayaan lainnya.",
  ...
  useEffect(() => {
    fetch("/api/templates", { cache: "no-store" })
      .then(async (response) =>
        response.ok ? response.json() : { templates: [] },
      )
      .then((data) =>
        setRemoteTemplates(
          (data.templates ?? []).map(
            (template: {
              ...
              key: template.key,
              name: template.name,
              theme: template.description || copy.custom,
    ...
    <section className="space-y-9">
      <div className="flex flex-col justify-between gap-6 border-b border-border/70 pb-7 sm:flex-row sm:items-end">
        ...
        <Button
          asChild
          size="sm"
          className="min-w-0 shrink-0 text-xs uppercase tracking-[0.14em]"
        >
          ...
          <article key={template.key} className="group w-full min-w-0">
            <div className="relative mx-auto aspect-[0.72] w-full max-w-[270px] overflow-hidden rounded-[28px] border-2 border-[#111111] bg-white p-2 shadow-lg shadow-black/5 transition-transform duration-500 group-hover:-translate-y-1.5 dark:border-white dark:bg-[#111113] dark:shadow-black/20">
              ...
              <span className="absolute left-5 top-5 border border-white/30 bg-black/35 px-2.5 py-1 font-[family-name:var(--font-dc-mono)] text-[8px] uppercase tracking-[0.16em] text-white backdrop-blur-md">
                {template.templateNo ? `#${template.templateNo}` : copy.preview}
              </span>
            </div>
            <div className="mx-auto max-w-[270px] pt-5">
              <div className="flex items-baseline justify-between gap-4">
                ...
                <span className="font-[family-name:var(--font-dc-mono)] text-[8px] uppercase tracking-[0.15em] text-foreground/40">
                  {template.templateNo ? copy.custom : copy.invitation}
                </span>
              </div>
              <p className="mt-1 font-[family-name:var(--font-dc-body)] text-xs text-foreground/55">
                {template.theme}
              </p>
              <Button
                type="button"
      ...
      {selected && (
        <div
          ...
          role="dialog"
          aria-modal="true"
          aria-label={`${copy.preview}: ${selected.name}`}
          onClick={() => setSelected(null)}
        >
          <div
            className="relative flex max-h-[92vh] w-full max-w-[430px] flex-col items-center gap-5"
          ...
          >
            <div className="relative aspect-[0.48] w-[min(82vw,330px)] rounded-[38px] border-[7px] border-[#171719] bg-[#171719] p-[5px] shadow-[0_24px_70px_rgb(0_0_0_/_0.4)] dark:border-[#e5e5e5] dark:bg-[#111113]">
              <div
                className="absolute -right-[10px] top-[23%] h-14 w-[4px] rounded-r-full bg-[#303034]"
                aria-hidden="true"
              />
              <div
                className="absolute -left-[10px] top-[21%] h-8 w-[4px] rounded-l-full bg-[#303034]"
                aria-hidden="true"
              />
              <div
                className="absolute -left-[10px] top-[31%] h-12 w-[4px] rounded-l-full bg-[#303034]"
                aria-hidden="true"
              />
              <div className="relative h-full overflow-hidden rounded-[30px] bg-white">
                <div
                  className="absolute left-1/2 top-2 z-20 h-6 w-[34%] -translate-x-1/2 rounded-full bg-black"
                  aria-hidden="true"
                />
                <div className="h-full overflow-y-auto scrollbar-thin">
                  <Image
                    ...
                    width={900}
                    height={1500}
                    className="min-h-full w-full object-cover object-top"
                    priority
                  />
                </div>
              </div>
            ...
            <div className="flex w-full max-w-[330px] items-center justify-between gap-3">
              <div className="min-w-0 text-white">
                <p className="font-[family-name:var(--font-dc-mono)] text-[9px] uppercase tracking-[0.2em] text-white/65">
                  {copy.preview}
                </p>
                <p className="truncate font-[family-name:var(--font-dc-heading)] text-xl">
                  {selected.name}
                </p>
              </div>
