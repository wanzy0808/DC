...
## Landing-only brand readability (23 September 2026)

On `/` and `/pagecontoh`, the embedded public `Navbar` selects the dedicated `BrandWordmark size="landing"` so the existing Cinzel brand name is 34px from `sm` and 36px from `lg` (instead of the shared public 30px), and the original tagline grows modestly from 8px to 9px/10px at the same breakpoints. Narrow mobile retains the old sizing so the right-hand theme/language/burger controls remain reachable. Landing-only vertical navbar padding changes from `py-5` to `py-5 sm:py-4` to keep the overall navbar height from increasing. The reusable public/dashboard/mobile wordmark sizing remains unchanged on all other pages, as do the actual wording, font, palette, background, burger menu, 3D doors, cloud text, footer, marketing ambience and auth dialogs. Verify the result on the owner's real desktop/mobile viewport after syncing; a successful build does not confirm visual fit.
