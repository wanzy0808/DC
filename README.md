...
## Zen Atelier — tambahan tema tanpa foto (24 September 2026)

`zen-atelier` adalah tema READY ke-11, melengkapi sepuluh tema yang sudah ada tanpa mengubah key undangan lama. Gunakan katalog di `/template-design?template=zen-atelier`, kemudian Studio memilihnya melalui registry yang sama. Tema ini menggunakan palet Zen (krem, sage, terakota), Playfair Display + Inter, amplop surat interaktif, sakura/ensō/pegunungan tinta dalam ilustrasi vektor, section tanpa foto, dan semua 15 toggle lama (amplop + 13 bagian + musik). RSVP live tetap memakai `RsvpForm` bersama; Wishes tetap placeholder sampai backend bersama tersedia. Musik pengguna dan Sharp WebP untuk foto tema lain tidak berubah.

Source artwork berada di `assets/templates/zen-atelier/` (bukan `public/`), lazy-loaded hanya untuk tema Zen Atelier. Thumbnail adalah turunan SVG di `/api/template-preview/zen-atelier`. **Repo ini public; source art di luar public bukan private dan artwork yang tampil di browser bisa disalin.** Jangan commit master image beresolusi tinggi ke repo ini. Simpan master di private storage di luar Git dan web root, beri izin server-side untuk akses master. Aturan .gitignore untuk folder `private-assets/` hanya mencegah commit baru secara tidak sengaja; tidak menghapus Git history. Source foto/ilustrasi asli dari link chat belum teridentifikasi dalam main per 24 September; ilustrasi vektor baru adalah interpretasi moodboard, bukan salinan/migrasi file asli. Lihat `assets/templates/zen-atelier/README.md` untuk alur penyimpanan.

## Ten visually distinct invitation themes (5 with photos / 5 photo-free) — baseline 22 September 2026
...
### Flat dashboard sections instead of cards inside cards (23 September 2026)

`DashboardPanel` now renders a flat section with a text heading/actions and light divider on the dashboard canvas. It does not wrap nested metric/list/detail cards in another Rose-framed surface. This shared change covers Invitations, Personal Invitations, WA Blast, RSVP, Guest Management/Seating and Usher; the Event list uses a flat section as well. Empty event selection/states no longer gain a second wrapper, and the Personal Invitation password editor is an inline divider-separated row, not an inner card. Individual meaningful cards continue using the existing uniform `0.3cm` left Rose stripe, top-right-only radius, neutral Light/Dark surfaces and real data/actions. Standalone account form cards, Beranda summary panels with plain internal rows and the functional seating stage remain. External mainframe, public landing/Pintu and scrolling behavior unchanged. Browser/mobile/CI validation remains pending.


### Dashboard latest layout: few large frames, no nested mini-cards (23 September 2026)

**This supersedes the preceding flat-section/small-card iteration.** `DashboardPanel` now provides one large Rose-framed section containing its heading/actions and plain unframed list rows. `DashboardMetricGrid` groups all numerical stats into one large frame with unframed metric columns. Nested event/invitation/RSVP/guest/WA recipient and template entries are plain rows, with thin separators rather than their own card fills, radii, shadows or `0.3cm` left stripes. The shared `0.3cm` left stroke and top-right-only round corner belong to **large outer section frames**, not internal records. Beranda retains a few peer-level large panels, standalone account forms remain large, and the seating canvas no longer has an extra nested border; functional inputs, buttons, badges and charts retain their appropriate affordances. Existing data, outer mainframe, inner scroll, WhatsApp support, dark/light themes and public landing/Pintu unchanged. App commits `5cd96a41`, `11c2f963`, `cdc162ca`, `e3b2e472`, `99621037`; browser/CI verification still pending.
