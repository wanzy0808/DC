...
- `components/Landing/Pintu/`, `components/Layout/`, `components/Brand/`, `components/Marketing/` — landing portals, public shell, shared brand and marketing UI. The production four-door landing uses `components/Landing/Pintu/SimpleDoorLab.tsx` and `PortalTransition.tsx`; the old `/jiplak`, `/pintu-lab` preview components and their engines are no longer present.
