...
- Dark Dashboard header utilities follow the same neutral-first rule: ID/EN and the account trigger/dropdown remain near-black with white copy while idle; Rose appears only on hover/interaction. The selected language uses neutral white emphasis rather than an idle Rose fill.
