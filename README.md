...
- Dashboard header controls use one shared visual family across themes: burger, theme toggle, ID/EN, and account trigger share the same compact rounded shape, border, and Rose hover behavior. Light Mode uses a white/neutral idle surface with Rose foreground; Dark Mode uses near-black with white foreground. Selected language uses border/underline emphasis rather than a separate fill style.
