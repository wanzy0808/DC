...
- `components/InvitationStudio/` — active invitation editor/studio components. `InvitationDesigner.tsx` owns orchestration; `DesignerPanels.tsx` contains controls; `InvitationPreview.tsx` renders the canvas; adjacent `designer-*` modules hold types/config/state helpers. Guest management follows the same pattern with `GuestManagement.tsx` as orchestration and adjacent `GuestManagementPanels.tsx`, `guest-management-types.ts`, and `guest-management-client.ts`.
