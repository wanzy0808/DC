...
`/login` uses a centered responsive Rose-outlined form matching the public site in both themes, with shared pill inputs/buttons, an accessible show/hide password control, and ID/EN labels through LanguageProvider. The navbar's existing Register dialog uses the same `components/Auth/auth-styles.ts` / `GoogleIcon.tsx` elements, a scrollable mobile-friendly modal, labelled password/confirmation fields and retained Terms/Privacy consent. The Login page's Daftar link opens this real dialog using `/login?register=1&next=...`, and Masuk from the navbar dialog closes it and navigates to login. Auth APIs, existing Google route, redirect roles and invitation entitlements are unchanged. `PublicContent` only allows full responsive width for the /login route; marketing chrome and approved landing remain untouched.
