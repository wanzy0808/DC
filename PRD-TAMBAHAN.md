...
### Commit
- `5ead256324e6c5d1e07c66066eb58... (truncated)`.
### Validation
- Source-of-truth docs and landing implementation reviewed.
...
## 36. Right Burger Menu Refinement
### User Requirement
Menu burger di kanan perlu dibenahi agar panel navigasi terasa lebih rapi, proporsional, dan nyaman digunakan.
### Implemented
- `components/Layout/Navbar/BurgerMenuContent.tsx` diperbaiki tanpa mengganti arsitektur Sheet yang sudah ada.
- Lebar panel sekarang responsif (`min(92vw, 430px)`) dan tidak lagi terbatasi `sm:max-w-sm` yang membuat ukuran desktop lebih sempit dari intent sebelumnya.
- Panel memakai border, shadow, background transparan terkontrol, dan backdrop blur ringan untuk menyatu dengan visual system DC Organizer.
- Konten menu menjadi scrollable sementara footer tetap berada di bagian bawah panel.
- Item navigasi memiliki tinggi minimum yang lebih konsisten dan target interaksi yang aman.
- Section `Layanan` mendapat border separator dan `aria-expanded` pada toggle.
- Deskripsi layanan dipotong aman dengan `truncate` agar tidak merusak layout panel sempit.
- Typography tetap menggunakan font system DC Organizer dan tidak menambah dependency.
- Route `/login`, `/packages`, `/template-design`, `/help`, serta tiga route Pintu tetap dipertahankan.
- `SheetClose`/Base UI composition tetap dipertahankan untuk menghindari perubahan behavior navigasi.
### Commit
- `18be23426280d624131b9cce6aa52634c8d1e4f0` — `fix(nav): refine right burger menu panel`.
### Validation
