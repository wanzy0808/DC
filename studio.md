...
- Kontrol aksi Studio memakai keluarga tombol DC Organizer yang sama: bentuk **rounded rectangle** dengan `--dc-control-radius` 16px, bukan pill. Pada toolbar canvas, **Restart, Undo, Redo, dan Simpan harus memakai bentuk/ukuran tombol yang sama**; perbedaannya hanya ikon, label, status disabled, dan fungsi. Jangan menambah varian bentuk/warna tombol yang berbeda untuk setiap halaman.
