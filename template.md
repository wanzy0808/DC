...
**Status implementasi existing (bukan pengecualian permanen untuk template baru):** tema yang sudah READY saat aturan ini ditulis baru menyediakan slot `greeting` dan `closing`; Zen Atelier pada acara berformat pasangan juga mempunyai `zenQuote`. Slot `attendanceRequest` dan `prayerWish` belum tersedia sebagai field terpisah pada semua renderer lama. Untuk template baru, implementasikan slot nyata beserta registry `lib/templates/editable-copy.ts` dan renderer sebelum menampilkannya di menu Isi; lakukan audit/migrasi UI templat lama secara bertahap tanpa menghilangkan copy yang sudah tersimpan. Jangan menyatakan empat slot wajib sudah berfungsi di semua tema lama sebelum integrasi dan tes selesai.
...
- [ ] Setiap template baru memiliki komponen narasi nyata untuk salam, permohonan kehadiran, doa/harapan, dan penutup yang terhubung ke menu Isi; teks penjelasan/kutipan hanya diedit apabila benar-benar dirender. Semua default tampil wajar, dan perubahan langsung terlihat di canvas, ikut Undo/Redo, tersimpan untuk satu event serta sama pada undangan tamu; tidak ada input duplikat data acara.
