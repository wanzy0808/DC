...
**Menu Isi adalah editor kata-kata undangan, bukan form acara kedua.** Untuk tiap template baru, desainer dan developer harus mendeklarasikan hanya slot teks naratif yang **benar-benar muncul dalam renderer** dan boleh diedit pengguna: misalnya salam pembuka/permohonan kehadiran, doa atau kutipan jika section tersebut memang ada, dan ucapan penutup. Beri nama dan batas karakter per slot, serta nilai default sesuai karakter template. UI Studio hanya menampilkan field yang didukung tema dan kategori acara saat itu; jika sebuah tema tidak memiliki slot doa/kutipan, jangan tampilkan input kosong yang tidak mengubah hasil.
...
**Implementasi awal renderer tersedia:** seluruh tema siap-render menyediakan `greeting` dan `closing`; Zen Atelier untuk acara berformat pasangan (pernikahan atau ulang tahun pernikahan) juga menyediakan `zenQuote` pada kutipan penutup. Tidak ada input doa universal baru yang menghasilkan paragraf fiktif pada tema tanpa slot; perlu menambahkan slot nyata ke renderer + registry terlebih dahulu apabila tema baru meminta doa tambahan.
