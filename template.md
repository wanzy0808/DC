...
**Cerita pasangan / Our Story** (`ourStory`) adalah slot narasi tambahan **wajib tersedia pada setiap tema baru yang mendukung acara berformat pasangan**, tampil sebagai segmen tersendiri setelah identitas pasangan dan sebelum detail acara. Field ini dapat diedit langsung dari text pada canvas Studio melalui inspector kanan, termasuk kalimat panjang dan baris baru, dengan batas maksimal 1.600 karakter. Judul tampilan **Our Story / Tentang Kami** dan dekorasinya milik template; **nama pasangan tetap berasal dari data acara**. Ketika cerita belum diisi, jangan munculkan cerita karangan, teks contoh, atau area kosong pada undangan tamu. Segmen mengikuti toggle **Identitas**, tanpa menciptakan section ke-16 atau toggle baru. Untuk acara non-pasangan, menu Isi tidak menampilkan field ini dan renderer tidak menampilkan segmen cerita pasangan. Cerita disimpan per event di `::copy=` dan menggunakan renderer yang sama pada canvas Studio dan undangan publik.
...
**Menu Isi bukan form acara kedua.** Menu ini menjadi navigator struktur section: nama section + toggle ON/OFF, dan child **Input/Button** hanya bila komponen fungsional tersebut benar-benar ada pada renderer. Text naratif dan Image dipilih langsung dari canvas; panel kanan mengatur text/style visual, sedangkan fungsi/data bisnis tetap bersumber dari model event/API yang sama. Setiap slot narasi di atas harus mempunyai teks default, batas karakter, identitas field stabil pada manifest/capability, dan jalur renderer nyata → canvas Studio → Simpan Desain → undangan publik/personal yang sama. Penggantian teks harus langsung terlihat serta ikut Undo/Redo; field yang tidak tersedia pada kategori acara/varian template disembunyikan, bukan menghasilkan input palsu. Jangan mengubah teks nama, tanggal, venue, data tamu atau label tetap milik komponen. **Template baru tidak dianggap READY jika slot narasi wajib hanya muncul sebagai input Studio tetapi belum muncul pada undangan sungguhan.**
...
**Status implementasi existing:** seluruh tema READY kini mempunyai slot `greeting`, `closing`, serta **`ourStory` khusus event berformat pasangan**; Zen Atelier pada acara berformat pasangan juga mempunyai `zenQuote`. Slot `attendanceRequest` dan `prayerWish` belum tersedia sebagai field terpisah pada semua renderer lama. Untuk template baru, implementasikan slot nyata beserta registry `lib/templates/editable-copy.ts` dan renderer sebelum menampilkannya di menu Isi; lakukan audit/migrasi UI templat lama secara bertahap tanpa menghilangkan copy yang sudah tersimpan. Jangan menyatakan empat slot wajib sudah berfungsi di semua tema lama sebelum integrasi dan tes selesai.
