...
## 23 September 2026 — Unified Personal Invitation form and canonical Guest

- Replaced the two duplicate entry areas with one create form: choose an existing guest or add one; main fields are name/optional phone, recipient type, invited capacity and Regular/VIP/VVIP category; optional envelope name, grouped guest tags and a greeting are tucked under additional settings. List editing reuses the same profile fields. This does not introduce smaller nested cards; the approved large-frame dashboard is preserved.
- Canonical DB source remains `Guest.id` (event scoped) for personal invitation, RSVP, seating, WA Blast and Usher. Reuse existing category/tags/name/phone and RSVP/seat/check-in fields; add only `personalAddressee`, `recipientType`, `invitedPax`, `personalGreeting`, `personalSharedAt` columns on Guest. Existing invitations/RSVP remain; backfill previously confirmed companions to avoid downgrading their quota.
- APIs create a single Guest on genuinely new input or update an existing Guest in place, with backend validation and duplicate-name+phone rejection. The personal route passes existing Guest ID and unique token to all public render-ready templates' RSVP form; the API checks token/publish/quota and UPDATES that guest, rather than creating another attendee row. Public and owner-preview display the envelope name/greeting; record/status views show invitation capacity, guest labels, RSVP/check-in/table and manual sharing marker. Guest list endpoint exposes fields to other dashboard tabs; RSVP confirmed pax now excludes nonattendees.
- Important: `invitedPax` is max invitees, `plusOnes` is actual RSVP companions; they are not synchronized or copied. Mark-as-shared is only a manual note, **not** WhatsApp send/delivery telemetry. Current seating assigns one Guest per seat; a true per-person group seating model is separate future work, not duplicate Guest entries. No bulk import, delivered WA, or category-based reserved seats were implemented in this iteration.
- Run `pnpm db:deploy` then `pnpm db:generate` after syncing. CI builds run Prisma generate, but authenticated browser/real database migrations/RSVP end-to-end still require confirmation.


## 23 September 2026 — Cross-workspace guest state and selected event

- Guest identity/profile are stored once on the original event's Guest record. Reused across Undangan Personal, generic/personal RSVP, WA Blast recipient queue, seated guests, guestbook/Usher and overview totals. Shared contact match prevents new intake duplicates and authenticated WA reuses existing unique recipient; contact-only public RSVP refuses edits to an existing record without a personal token.
- All five relevant customer dashboard views share `activeEventId`, fetch the selected event's Guest records when opened, and leave the established large single-frame dashboard layout unchanged.
- Embedded Usher has an event picker and links to `/dashboard/usher?invitationId=...`; its standalone guest listing, QR scan and legacy endpoint verify the selected/actual guest event before check-in. The canonical check-in is atomic and carries the actor/timestamp; public responses do not expose sensitive guest hashes.
- Pre-existing Guest personal-profile/backfill migrations verified in the repo. Applying them on the user's deployed database and live browser QA are distinct steps; do not report either done without verification. Group invitations still represent one Guest for seating; attendee-by-attendee seats are not yet implemented.
