      ...
      <main className="relative z-10 mx-auto flex w-[75vw] max-w-[1400px] flex-1 items-center px-0 pt-6 pb-5 lg:pt-8 lg:pb-6">
        <section className="grid w-full grid-cols-1 items-center gap-3 md:grid-cols-12 lg:gap-6">
          <div className="space-y-2 md:col-span-5 lg:space-y-4">
            <div key={isIdle ? "idle" : selectedDoor} className="animate-in fade-in slide-in-from-bottom-3 space-y-2 duration-500 ease-out lg:space-y-4">
              {isIdle ? (
                <>
                  <span className="block font-mono text-[15px] font-semibold uppercase tracking-[0.24em] text-[var(--primary)] sm:text-xs">
                    DC Organizer · Platform Digitalisasi Pernikahan Terpadu
                  </span>
                  <h1 className="flex max-w-xl flex-wrap gap-x-[0.25em] gap-y-1 font-[family-name:var(--font-dc-heading)] text-2xl leading-[1.15] tracking-tight text-[var(--foreground)] sm:text-3xl lg:text-4xl">
                    {idleWords.map((word, index) => <span key={index} className="dc-snappy-word inline-block" style={{ animationDelay: `${index * 45}ms` }}>{word}</span>)}
                  </h1>
                  <p className="max-w-lg text-sm leading-relaxed text-[color:var(--muted-foreground)] sm:text-base">Buka salah satu pintu di kanan untuk melihat. Biarkan kami membantu mewujudkan hari bahagiamu.</p>
                </>
              ) : (
                <>
                  <span className="block font-mono text-[10px] font-semibold uppercase tracking-[0.22em] text-[var(--primary)] sm:text-xs">{currentContent.badge}</span>
                  <h1 className="max-w-xl font-[family-name:var(--font-dc-heading)] text-2xl leading-[1.15] tracking-tight text-[var(--foreground)] sm:text-3xl lg:text-4xl">{currentContent.title}</h1>
                  <p className="max-w-lg text-sm leading-relaxed text-[color:var(--muted-foreground)] sm:text-base">{currentContent.description}</p>
                  <ul className="space-y-1.5 pt-1">{currentContent.features.map((item, idx) => <li key={idx} className="flex items-center gap-2 text-sm text-[color:var(--muted-foreground)] sm:text-base"><span className="inline-flex h-1.5 w-1.5 shrink-0 rounded-full bg-[var(--primary)]" /><span>{item}</span></li>)}</ul>
                  <div className="my-1 border-l-2 border-[var(--primary)] py-1 pl-3"><p className="text-sm italic leading-relaxed text-[var(--foreground)] sm:text-base">“{currentContent.quote}”</p><span className="mt-1 block font-mono text-[10px] uppercase tracking-[0.16em] text-[color:var(--muted-foreground)]">— {currentContent.quoteAuthor}</span></div>
                  <Link href={currentContent.href} className="inline-flex pt-1"><Button className="rounded-full bg-primary px-5 py-3 text-xs font-semibold uppercase tracking-[0.18em] text-primary-foreground shadow-lg transition hover:bg-primary/85">{currentContent.buttonText}</Button></Link>
                </>
              )}
            </div>
          </div>
          <div className="flex items-center justify-center md:col-span-7"><PintuSection activeDoor={activeDoor} setActiveDoor={setActiveDoor} /></div>
        </section>
      </main>
    </div>
  );
