    ...
    <main className="relative z-10 min-h-screen overflow-hidden px-5 pb-20 pt-24 text-[var(--foreground)] sm:px-8">
      ...
      <div className="relative mx-auto max-w-6xl space-y-28">
        <header className="flex flex-col gap-5 border-b border-[var(--border)] pb-7 md:flex-row md:items-end md:justify-between">
          <div>
            <p className="text-xs font-mono font-semibold uppercase tracking-[0.24em] text-[var(--primary)]">[ DC WEDDING ]</p>
        ...
        <section className="rounded-3xl border border-[var(--primary)]/20 bg-[var(--primary)]/[0.06] p-7 md:p-10">
          <div className="grid gap-7 md:grid-cols-[1fr_auto] md:items-center">
            <div><p className="text-xs font-mono uppercase tracking-[0.2em] text-[var(--primary)]">[ DIGITAL WORKFLOW ]</p><h2 className="mt-3 font-[family-name:var(--font-dc-heading)] text-3xl">Planning yang nyambung dengan tamu dan undangan</h2><p className="mt-3 max-w-2xl text-sm leading-7 text-[var(--muted-foreground)]">Jika Anda membutuhkan lebih dari koordinasi vendor, DC dapat menggabungkan workflow planner dengan Undangan Digital dan Guestbook Digital untuk RSVP, seating, greeting, sampai check-in.</p></div>
        ...
        <section className="rounded-3xl bg-[var(--primary)] p-8 text-white md:p-12"><p className="text-xs font-mono uppercase tracking-[0.2em] opacity-75">[ READY WHEN YOU ARE ]</p><h2 className="mt-3 max-w-2xl font-[family-name:var(--font-dc-heading)] text-3xl md:text-4xl">Mari mulai dari cerita dan kebutuhan Anda.</h2><p className="mt-4 max-w-2xl text-sm leading-7 opacity-85">Tidak perlu sudah punya semuanya. Kami bisa mulai dari tanggal, venue, konsep yang Anda bayangkan, lalu menentukan prioritas berikutnya.</p><Link href="https://wa.me/6281234567890?text=Halo%20DC%20Wedding,%20saya%20ingin%20konsultasi%20Wedding%20Planner" target="_blank" className="mt-7 inline-flex items-center gap-2 rounded-full bg-white px-6 py-3 text-xs font-semibold uppercase tracking-wider text-[var(--primary)]">Mulai konsultasi <ArrowRight className="h-4 w-4" /></Link></section>
      </div>
    </main>
  );
}
