    ...
    <main className="relative z-10 min-h-screen w-full overflow-hidden px-0 pb-20 pt-24 text-[var(--foreground)]">
      ...
      <div className="relative mx-auto w-full space-y-28">
        <header className="flex flex-col gap-5 border-b border-[var(--border)] pb-7 md:flex-row md:items-end md:justify-between">
          <div>
            <p className="font-mono text-xs font-semibold uppercase tracking-[0.24em] text-[var(--primary)]">[ DC ORGANIZER ]</p>
          ...
          <Button asChild size="lg" className="w-fit rounded-xl">
            <Link href="/login"><User className="h-4 w-4" />Client Login</Link>
          </Button>
        </header>
        <FounderSection />
        <ServicesSection />
        ...
        <section className="rounded-3xl border border-[var(--primary)]/20 bg-[var(--primary)]/[0.06] p-7 md:p-10">
          <div className="grid gap-7 md:grid-cols-[1fr_auto] md:items-center">
            <div>
              <p className="font-mono text-xs uppercase tracking-[0.2em] text-[var(--primary)]">[ DIGITAL WORKFLOW ]</p>
              <h2 className="mt-3 font-[family-name:var(--font-dc-heading)] text-3xl">Planning yang nyambung dengan tamu dan undangan</h2>
              <p className="mt-3 max-w-2xl text-sm leading-7 text-[var(--muted-foreground)]">Jika Anda membutuhkan lebih dari koordinasi vendor, DC dapat menggabungkan workflow planner dengan Undangan Digital dan Guestbook Digital untuk RSVP, seating, greeting, sampai check-in.</p>
            </div>
            <Button asChild size="sm" className="rounded-xl">
              <Link href="/d-invitation">Lihat produk digital <ArrowRight className="h-4 w-4" /></Link>
            </Button>
          </div>
        </section>
        ...
        <ReviewsGrid eyebrow="Client Stories" title="Hari-H yang terasa lebih ringan" description="Pengalaman pasangan setelah mempercayakan koordinasi dan planning kepada DC Organizer." reviews={plannerReviews} />
        ...
        <section className="rounded-3xl bg-[var(--primary)] p-8 text-white md:p-12">
          <p className="font-mono text-xs uppercase tracking-[0.2em] opacity-75">[ READY WHEN YOU ARE ]</p>
          <h2 className="mt-3 max-w-2xl font-[family-name:var(--font-dc-heading)] text-3xl md:text-4xl">Mari mulai dari cerita dan kebutuhan Anda.</h2>
          <p className="mt-4 max-w-2xl text-sm leading-7 opacity-85">Tidak perlu sudah punya semuanya. Kami bisa mulai dari tanggal, venue, konsep yang Anda bayangkan, lalu menentukan prioritas berikutnya.</p>
          <Button asChild size="lg" className="mt-7 rounded-xl bg-white text-[var(--primary)] hover:bg-white/90 dark:bg-white dark:text-[var(--primary)] dark:hover:bg-white/90">
            <a href="https://wa.me/6281234567890?text=Halo%20DC%20Organizer,%20saya%20ingin%20konsultasi%20Wedding%20Planner" target="_blank" rel="noreferrer">Mulai konsultasi <ArrowRight className="h-4 w-4" /></a>
          </Button>
        </section>
      </div>
    </main>
