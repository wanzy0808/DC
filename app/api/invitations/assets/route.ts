...
export async function POST(request: Request) {
  ...
  try {
    ...
    const invitation = await prisma.invitation.findFirst({ where: { id: String(body.invitationId), ownerId: user.id }, include: { payment: true } });
    ...
    if (!url || !/^https?:\/\//i.test(url)) {
      return NextResponse.json({ error: "Asset harus berupa URL http atau https." }, { status: 400 });
    }
