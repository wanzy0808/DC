...
## 23 September 2026 — WA Blast template pesan (konten referensi saja)

Implementasi customer dashboard sekarang mendukung pembuatan/edit/hapus template Undangan, Pengingat RSVP, Pengingat hari acara, dan Ucapan terima kasih per acara menggunakan model `WaBlastTemplate`, migration, dan API owner-scoped. Template memuat nama, judul, isi, kategori, placeholder `{nama}`, `{acara}`, `{tanggal}`, `{lokasi}`, `{link}`, serta pratinjau penerima nyata/label contoh saat belum ada penerima. UI mengikuti Rose dashboard dan tidak meniru tampilan magenta situs referensi. Tautan hanya dapat disalin apabila undangan terbit; menyalin pesan bukan pengiriman massal dan tidak mengurangi kuota. Kuota tetap khusus untuk daftar penerima yang sudah ada; fitur sending & scheduling backend belum dibangun. Migrasi database wajib: `pnpm db:deploy` setelah sync. Checklist/QA di `Dashboard-redesign.md`.


## 24 September 2026 — Pencil Reverie: redesign undangan ilustrasi tanpa foto

Permintaan owner: hasil renderer sebelumnya terlalu generik, animasi tidak terasa, banyak aset tidak dipakai, dan perlu lebih mendekati moodboard Pencil Reverie (ilustrasi pensil + scrapbook oldies romantis remaja), tanpa mengubah desain template lain.

- Stable key \`pencil-reverie\`, kategori **Tanpa foto**, aset di \`public/templates/pencil-reverie/\` (sepuluh ilustrasi asli dipertahankan). Satu template dipakai ulang memakai data acara milik pengguna; foto upload tidak diwajibkan atau direkayasa. Galeri disebut **Galeri Cerita** dan memakai ilustrasi bawaan, bukan memalsukan foto pelanggan.
- Pembuka adalah scrapbook ilustratif (Polaroid, kaset, pita, tiket, kamera, balon), **bukan** wajib amplop fisik. Aksi utama tetap satu \`Buka Undangan\`; pemutar musik bersama dipanggil pada gestur klik dan crossfade/pembukaan berlangsung 1,05 detik sebelum masuk Cover.
- Cover punya tipografi editorial yang adaptif terhadap nama panjang, ilustrasi pasangan, sepeda, lampu jalan, sketsa kota, balon, buku, kamera, tiket, doodle hati, dan jam pensil. Section Greeting, Identity, Event, Date/Time, Gallery, Countdown, Location, RSVP, Wishes, Gift, Closing memperoleh komposisi aset berbeda, bukan satu pita sudut berulang. Narasi Greeting, acara, hadiah dan penutup memakai tata letak kiri/asimetris; beberapa bagian tetap center sesuai konsep.
- Animasi: hati digambar ulang, balon melayang, kaset berputar, lampu berdenyut halus, kolase muncul bertahap, Polaroid galeri terungkap per item, jam bergerak mundur; heading dapat replay setelah keluar dan masuk viewport. Loop dekoratif di-pause saat Cover di luar layar. Semua efek non-esensial mati pada \`prefers-reduced-motion:reduce\`; informasi tetap terlihat.
- Seluruh bagian data RSVP, QR, tamu, Gift, navigasi, section ON/OFF, dan Music memakai renderer/layanan lama tanpa duplikasi database. Komponen tema/artwork lazy-loaded untuk menghindari unduhan semua tema sekaligus; gambar dekoratif memakai Next Image untuk turunan web. Original PNG di Git tidak dihapus.
- Tests statis ditambahkan di \`tests/pencil-reverie.test.mjs\`. Belum ada klaim screenshot pixel-identical atau build/browser mobile tervalidasi dalam sesi ini; lakukan CI/build, cek pratinjau kartu Cover, Studio Amplop, 360–390px HP, desktop, reduced motion, nama panjang dan status RSVP sebelum siap produksi.


## 24 September 2026 — Pencil Reverie: full-art redesign berdasarkan audit aset

Pemilik meminta redesign lagi karena beberapa gambar tampil terpotong tanpa sengaja (terutama lampu yang hanya menyisakan gagang), penggunaan aset belum merata, dan komposisi masih jauh dari moodboard. Implementasi scoped `pencil-reverie`, bukan perubahan ke landing, Dashboard atau template lain.

- Repo memiliki **15 aset** untuk Pencil Reverie. Lima gambar baru `bingkai.png`, `bungaandlampbg.png`, `bungabg.png`, `bungabg1.png`, `sepedabg.png` adalah ilustrasi RGB rasio 1122×1402. Aset sumber dan blob SHA dicocokkan dengan file unggahan. Sepuluh PNG lama tetap ada dan digunakan secara terpilih: pasangan/sepeda utuh berada di Cover, kamera/kaset/buku/balon/tiket/polaroid/pita dalam section/galeri; lampu transparan `streetlamp.png` tampil **utuh** di panel Lokasi, bukan offset negatif di pinggir.
- Pembuka sekarang surat pita `bingkai.png` lengkap dengan nama/tanggal nyata dan satu tombol Buka Undangan; transisi terjaga bersama audio/Studio. Cover memakai **seluruh** `bungaandlampbg.png` sebagai lanskap dengan lapisan ilustrasi pasangan dan sepeda yang seluruhnya tampak, nama dan tanggal dari event, serta scroll menuju section berikutnya. Kartu katalog memakai gambar Cover, bukan foto pasangan terpisah atau amplop.
- `PencilSectionArt` mengganti posisi absolut tiga aset generik per section dengan ilustrasi lengkap dalam flow setelah informasi section. Galeri Cerita berisi ilustrasi tematik bawaan (bukan klaim foto riwayat user) dengan lightbox keyboard dan gesture geser; jam hitung mundur tetap terhubung ke tanggal acara nyata dan jarumnya berputar mundur. CSS mengutamakan rasio utuh, responsive Next/Image, token palet/font, dan reduced motion.
- Menyesuaikan `template.md` tentang konten: Pencil Reverie kini punya slot teks nyata `greeting`, `attendanceRequest`, `prayerWish`, `closing`, dan `ourStory` hanya pada event berpasangan. Studio Isi dan renderer publik/Studio membaca kontrak copy event yang sama tanpa tabel DB baru; data nama, tanggal, venue, RSVP, QR, hadiah tetap dari sistem bersama.
- Ditambahkan aturan global ilustrasi tanpa crop acak ke `template.md` dan AGENTS.md, audit aset ke README tema serta tes statis per tema. **Belum** dijalankan build/tes otomatis atau dibandingkan screenshot browser dengan moodboard pada giliran ini; jangan menandai sebagai final sampai visual HP 360/390, desktop, tema terang/gelap custom, pembuka, keyboard/lightbox, RSVP, Studio dan lint/build lolos.
