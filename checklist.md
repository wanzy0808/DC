...
- [x] Commit pembersihan akhir `93db1101` telah lolos [Build Validation](https://github.com/wanzy0808/DC/actions/runs/36011178677) (termasuk tes) dan [Orphan Audit](https://github.com/wanzy0808/DC/actions/runs/36011178767). Kandidat yang tersisa hanya dua deklarasi `reference-door-*.d.ts` pendamping JS yang masih digunakan route lab dan satu primitive UI `components/ui/sheet.tsx`; **pertahankan**, jangan auto-delete berdasarkan import statis saja.
...
- [ ] Lanjut audit/refactor bertahap `InvitationDesigner.tsx` (load/save vs canvas/layer), `UniversalInvitationTemplate.tsx` (renderer/feature engine vs presentasi tema), operasi server tersisa di `app/api/invitations/route.ts`, `EventPanel.tsx` dan `SeatingChart.tsx` bila tanggung jawab terbukti bisa dipisahkan aman; pertahankan API/data event dan tes regresi.
